...
-- Release tagging. `$VersionNumber` is the single source of truth for the release: a Real defined in `src/version.h` as `MATHILDA_VERSION_NUMBER`, with its textual twin `MATHILDA_VERSION_STRING` (keep the two in sync by hand). It is surfaced in the REPL as `$VersionNumber` and embedded in `$Version`. **Every _substantive_ commit** — one that changes Mathilda's behaviour, capabilities, or output (a new/modified builtin, a bug fix, a semantics or performance change) — **bumps the version** in `src/version.h` (conventionally by `+0.001`), notes it in the commit message as before (`; v0.156`), and **is given a git tag `v<MATHILDA_VERSION_STRING>` pointing at that commit** (`git tag v0.156 && git push origin v0.156`, or commit then `git push --follow-tags`). Use the STRING, not the printed Real, as the tag name — the double `0.160` prints as `0.16`, but the tag must be `v0.160`. Tags are lightweight, for consistency with the backfill. **Purely contributor-facing commits do NOT bump and are NOT tagged**: CLAUDE.md / SPEC.md prose, CI or build tooling, comment- or changelog-only edits — none of these change `$VersionNumber`, so they ride the current version untagged. History through `v0.155` was backfilled (127 tags, one per version bump in `src/version.h`'s log); `git tag -l` lists them and `git describe --tags` names the nearest release.
