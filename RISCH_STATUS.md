# `Integrate`RischMacsyma` — Implementation Status

A faithful port of the algorithm structure of Maxima's `src/risch.lisp` — the
**recursive** Risch integration algorithm — into Mathilda.

- Source: `src/calculus/integrate_risch_macsyma.c` (+ `.h`)
- Tests: `tests/test_integrate_risch_macsyma.c`
...
Reachable as `Integrate`RischMacsyma[f, x]` (explicit backtick form) and
`Integrate[f, x, Method -> "RischMacsyma"]`, and inserted into the `Integrate`
...
| **Tower proper part** — Hermite + Rothstein–Trager, **log top** (`rm_field_ratint`) and **exp top** (`rm_field_hyperexp_coupled`) | ✅ |
| **Field Risch DE** in the lower field, **arbitrary (non-monomial) denominator** via the denominator theorem (`rm_field_rde`) | ✅ |
...
| **Multiplicatively commensurate merged kernels** (`E^(2 E^x) = (E^(E^x))^2` reduced to a primitive + integer powers in `rm_tower_build`) | ✅ |
| **Single-kernel pure resultant LRT** (algebraic residues → `ArcTan`/`Log` via `Res_t` + Rioboo `LogToReal`, `rm_frac_lrt`) | ✅ |
| **Tower-level resultant LRT**, **log top** (`rm_field_lrt_logpart` → `rm_field_ratint`; nested-log algebraic residues, e.g. `1/(x Log[x](Log[Log[x]]^2+1)) → ArcTan[Log[Log[x]]]`) | ✅ |
| **Tower-level resultant LRT**, **exp top** (`rm_field_lrt_logpart` → `rm_field_hyperexp_coupled`; e.g. `E^x E^(E^x)/(1+E^(2 E^x)) → ArcTan[E^(E^x)]`) | ✅ |
...
(`rm_field_ratint`, §3.4) — a nested-log proper part with algebraic residues
...
(`rm_field_hyperexp_coupled`): `E^x E^(E^x)/(1+E^(2 E^x)) → ArcTan[E^(E^x)]`.
...
1. **Recursive Risch, NOT parallel Risch.** This is the recursive
   decision-procedure algorithm (Maxima `risch.lisp`, in the Bronstein/Roach
   lineage). It is deliberately distinct from `Integrate`RischNorman`, which is
   the *parallel* Risch (Bronstein pmint / Norman) heuristic. RischMacsyma
...
4. **Grounded in Mathilda's `Expr`/poly/rat machinery**, not a reimplementation
   of Maxima's CRE. The transcendental extension is handled by aliasing each
...
`builtin_rischmacsyma` -> `rm_integrate`:

```
1. rm_rational_case         rational in x            -> delegate Integrate`BronsteinRational
2. rm_transcendental_case:
     a. rm_log_poly_case     polynomial in Log[u]
     b. rm_exp_poly_case     Laurent polynomial in E^u   (Risch DE per power)
     c. rm_frac_case         squarefree fractional log-part (Rothstein-Trager), log & exp kernels
                             — rational residues via SolveAlways, then rm_frac_lrt
                               (resultant LRT) for algebraic residues -> ArcTan/Log
     d. rm_hermite_case      repeated poles (Hermite),  log & exp kernels
     e. rm_hyperexp_case     coupled hyperexponential (mixed poly + log, Laurent-coupled Hermite)
     f. rm_expsum_case       sum of non-commensurate exponentials E^(W_k)  (decoupled Risch DE)
     g. rm_log_tower_case    nested logarithmic tower (depth > 1) over triangular D_tower
     h. rm_exp_tower_case    nested exponential tower (depth > 1), Laurent ansatz
     i. rm_recursive_tower_case  genuine one-extension recursion (mixed towers,
                                 rational lower-field coefficients)
     j. rm_trig_frontend     TrigToExp -> exp machinery (+ expsum) -> ExpToTrig
3. rm_special_case          Erf/Erfi, ExpIntegralEi, LogIntegral, PolyLog (Maxima erfarg/dilog/Ei/li)
```
...
### 3.1 Rational base case  (`rm_rational_case`)
Delegated to `Integrate`BronsteinRational` (Mathilda's recursive rational Risch:
Hermite + Lazard–Rioboo–Trager). This IS Maxima's rational Risch case.

### 3.2 Logarithmic-polynomial  (`rm_log_poly_case`)
...
oracle** (`rm_limited_integrate`) that folds a would-be new logarithm back into
...
### 3.3 Exponential Laurent-polynomial  (`rm_exp_poly_case`)
...
polynomial ansatz solved exactly with `SolveAlways` (`rm_solve_rde`); when `u` (the
...
(`rm_solve_rde_rational`), gated so `p_i`/`u'` must be genuine rational functions of
...
### 3.4 Fractional Rothstein–Trager log-part  (`rm_frac_case` / `rm_frac_try`)
...
**Pure resultant Lazard–Rioboo–Trager (`rm_frac_lrt`, 2026-07-11).**  The
...
`Log[x]^2+1` into a conjugate log pair) has no such solution, so `rm_frac_try`
declines.  When it does, `rm_frac_lrt` runs the exact resultant LRT: the residues
...
The exp kernel is `rm_exp_kernelize`d first so `E^(2x)` collapses onto `t^2`.
Because the reuse crosses content/denominator boundaries, this path is **diff-back
VERIFIED** (`rm_verify_antideriv`) — a mis-reduction declines, never ships wrong.
...
### 3.5 Hermite reduction (repeated poles)  (`rm_hermite_case` / `rm_hermite_try`)
...
### 3.6 Coupled hyperexponential  (`rm_hyperexp_case`)
...
### 3.9 Multi-kernel exponential sums  (`rm_expsum_case`) — Phase B, first increment
A sum `f = sum_k p_k(x) E^(W_k)` of exponentials whose exponents `W_k` are **not**
integer multiples of a single primitive (so `rm_exp_kernelize` cannot reduce them
...
solution of the Risch DE `q_k' + W_k' q_k = p_k` (`rm_solve_rde`, `W_k` polynomial
...
### 3.10 Nested logarithmic tower  (`rm_log_tower_case`) — Phase B, second increment
...
### 3.11 Nested exponential tower  (`rm_exp_tower_case`) — Phase B, third increment
...
**Both tower cases are diff-back VERIFIED** (`rm_verify_antideriv`): unlike the
...
**Single-kernel nesting gate (`rm_kernel_simple`).** A pre-existing correctness
bug: `rm_frac_case` (and the other single-kernel cases) substituted only ONE
...
rational in `x` alone (`rm_find_exp_of_x(u,x)==NULL && rm_find_log_of_x(u,x)==NULL`);
nested integrands fall through to the tower cases.

### 3.12 Genuine one-extension recursion  (`rm_recursive_tower_case`) — Phase B, fourth increment
...
is the genuine recursive Risch (Bronstein ch. 5 / Maxima `risch.lisp`): it peels
**one extension at a time**.

- `rm_tower_build` constructs the ordered differential tower (`RmTower`): collect
...
- `rm_field_integrate(F, L)` integrates `F` in `K_L`: `L < 0` is the rational base
  ...
  recursion `q_i' + (i+1) q_{i+1} Dt = p_i` (`rm_int_primitive_poly`), with each
  residual integrated by `rm_limited_field_integrate` (recursion + new-log fold-back
  of the single would-be new logarithm `= t_L`).  An **exponential** top uses the
  hyperexponential Laurent split (`rm_int_hyperexp_poly`): the `i = 0` term recurses,
  each `i != 0` term solves the Risch DE `q_i' + i w_L' q_i = p_i` (`rm_field_rde`,
...
- Correctness: a bounded search (not a decision procedure), so **diff-back verified**
  (`rm_verify_antideriv`); a spurious multi-variable certificate cannot ship.
...
**Proper rational part (fifth increment, 2026-07-11) — `rm_field_ratint`.** A
logarithmic top level with a genuine `t_n`-pole (proper fraction) is integrated
instead of declining: `rm_field_integrate` splits `F` into its `t_n`-polynomial
part (the primitive recursion) and its proper part, and `rm_field_ratint`
...
`D_tower[Q] = num/den` (`rm_tower_deriv`) over every tower variable via
...
**Tower-level resultant LRT (2026-07-11) — `rm_field_lrt_logpart`.** The
...
`rm_field_ratint` (and `rm_field_hyperexp_coupled`) fall back to the exact
resultant LRT — the same `Integrate`TranscendentalLogPart` used by the
single-kernel `rm_frac_lrt` (§3.4), but now with the *tower* derivation
...
`rm_field_hyperexp_coupled`.** The exponential-top counterpart of the fifth
...
invalid.  When `rm_int_hyperexp_poly` declines because a genuine `t_n`-pole is
...
**General field Risch DE (seventh increment, 2026-07-11) — `rm_field_rde` general
branch.** The exponential Laurent step (`rm_int_hyperexp_poly`) solves, per power
`i != 0`, the Risch DE `q_i' + i w_n' q_i = p_i` for `q_i` in the lower field.  Beyond
the base case (`C(x)`, polynomial `q_i` via `rm_solve_rde`), a `q_i` that is
...
The RDE solvers previously carried *arbitrary degree caps* — `rm_field_rde` used a loose
`deg(pd)+deg(pn)+1` proxy **capped at 5** plus a `nmono ≤ 60` monomial ceiling, and
`rm_solve_rde_rational` used `N = deg(pd)+deg(pn)+2` **capped at 10** — so a
...
in `rm_rde_var_bound(deg_v(p), deg_v(f), deriv_lowers)`.  For a Risch DE `D[q] + f q = p`,
...
(`nunk ≤ 60/80`) are now cap-free — `rm_log_tower_case`, `rm_exp_tower_case`, the
single-kernel Hermite and hyperexponential proper parts, `rm_field_hyperexp_coupled`,
...
top derivation coefficient `w' = D[u_n]` (derived via a new `rm_var_mult_at_zero` helper,
...
`rm_expand_exp_sums` + `rm_subst_kernels`.**  The evaluator automatically folds a
...
its argument `x + E^x` carries the foreign kernel `E^x`, so `rm_tower_build`'s
structure-theorem check rejects the tower and the case declines.  A structural
pre-pass `rm_expand_exp_sums` re-splits `E^(a+b) → E^a E^b` (built directly, *not*
...
(`rm_subst_kernels`) instead of by an evaluated `ReplaceAll`, which would re-merge
...
`rm_tower_build` commensurate reduction.**  Two collected exponents `w_i`, `w_j`
...
primitive's variable — new `RmTower.marg/mprim/mmult` fields populated in
`rm_tower_build` and consumed by `rm_subst_kernels`.  A class with no
...
exponential-top algebraic-residue LRT** (`rm_field_lrt_logpart`):
...
### 3.7 Trig / hyperbolic front-end  (`rm_trig_frontend`)
...
### 3.8 Special functions  (`rm_special_case`)  — Maxima erfarg / dilog / Ei / li
...
but `rm_frac_case` was wrongly closing it with a *polynomial-coefficient*
...
`rm_frac_try` used it without checking the residues are constant.  Fixed by
...
- `rm_eval_call` / `rm_eval_own` / `rm_eval1/2/3` — build-and-evaluate helpers
  (evaluate does NOT consume its argument; these own the constructed node).
- `rm_free_of_x`, `rm_free_of_head`, `rm_is_poly`, `rm_degree`
  (`= Length[CoefficientList] - 1`; Mathilda has no `Exponent` builtin),
  `rm_coeff`, `rm_is_zero` (`Together`-based, exact and cheap).
- `rm_template` — parse a template, `ReplaceAll` placeholder symbols, evaluate.
- `rm_find_log_of_x`, `rm_find_exp_of_x`, `rm_collect_exp_exponents`,
  `rm_exp_kernelize` (pick a primitive `u` so every `E^w` is `E^(k u)`,
...
Implemented by generalizing `rm_hyperexp_case`: the `theta`-coprime denominator
...
(`rm_expsum_case`, §3.9) — integrands that are a sum `sum_k p_k(x) E^(W_k)` of
...
(`rm_log_tower_case`, §3.10) — a rational function of a chain of nested logarithms
...
(`rm_exp_tower_case`, §3.11) via a Laurent ansatz over the exp tower derivation,
diff-back verified; plus the `rm_kernel_simple` fix closing a pre-existing
...
recursion** with a structure-theorem kernel basis (`rm_recursive_tower_case`,
...
(`rm_field_ratint`, §3.12): `H/Hden + sum c_j Log(g_j)` (constant residues).

**Sixth increment DONE (2026-07-11):** the **coupled hyperexponential proper part**
for an exponential top — `rm_field_hyperexp_coupled` (§3.12): one unified
...
`rm_field_rde` general branch (§3.12): finds a `q_i` rational in the lower field.
...
1. **Full Bronstein SPDE (Ch. 6).** *First increment DONE (2026-07-11) — principled,
   cap-free RDE degree bounds.*  Both RDE solvers (`rm_field_rde`,
   `rm_solve_rde_rational`) had **arbitrary degree caps** (cap-at-5 + `nmono ≤ 60`;
   ...
   `rm_rde_var_bound` — a function of the equation's degrees alone, monomial-type-aware
   ...
   flat-ansatz sites (`rm_log_tower_case`, `rm_exp_tower_case`, single-kernel Hermite +
   hyperexp, `rm_field_hyperexp_coupled`, hyperexp Hermite): exact top-kernel log/exp
...
2. ~~**Pure resultant Lazard–Rioboo–Trager.**~~ **DONE (2026-07-11).**  The
   ...
   (2026-07-11):** `rm_field_lrt_logpart` runs the same resultant LRT for the
...
3. ~~**Algebraically dependent kernels the evaluator merges.**~~ **DONE
   ...
   the `rm_expand_exp_sums` pre-pass (ninth increment), and the *multiplicatively
   commensurate* case (`E^(2 E^x) = (E^(E^x))^2`) by the **commensurate-exponent
   reduction in `rm_tower_build`** (tenth increment, §3.11): each commensurability
...
5. **Phase E — Maxima option/flag parity** (`$erfflag` / `$liflag` to force/suppress
   the special functions per call).  Low effort; option parsing only.
...
The exponential cases assumed `u` polynomial in `x`.  `rm_solve_rde` now defers,
when `u` or `p` is rational in `x`, to `rm_solve_rde_rational`, which solves
...
coefficient (the `Sin[x]` of a raw `E^x Sin[x]`, reached via `rm_exp_poly_case`)
...
### Phase E — Options / flag parity with Maxima
Expose `$erfflag` / `$liflag`-style options to force or suppress the special
functions per call (they are currently default-ON). Low effort; mostly option
...
```bash
make -j$(nproc)
cd tests && cmake -S . -B build && cmake --build build --target integrate_risch_macsyma_tests
./build/integrate_risch_macsyma_tests            # expect: All Integrate RischMacsyma tests passed!
valgrind --leak-check=full ./build/integrate_risch_macsyma_tests   # no leak stack through the module
```
