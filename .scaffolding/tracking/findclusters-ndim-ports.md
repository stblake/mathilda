...
ALL TEN METHODS ARE DIMENSION-GENERAL. Unified onto one kernel: DBSCAN, GaussianMixture. Two paths kept (the general rule moved a covered 1-D answer): KMeans, KMedoids, JarvisPatrick, Spectral. Already generic: Agglomerate, SpanningTree.
...
- [x] `Spectral` | `depends on: KMeans` | `done (iteration 8) -- needed NO k-means; components before Fiedler; mean-not-median threshold; fixed an n<3 segfault`
- [x] `GaussianMixture` | `depends on: prerequisites` | `done (iteration 9) -- kernel EXTRACTED to src/ml/gmm.c; unified (2nd after DBSCAN)`
...
- Existing tech debt noticed:
  ...
  - `1-D Spectral recomputes exp() for every pair on every power iteration instead of precomputing the affinity matrix. Harmless on a line, and the n-D kernel precomputes; unifying would be a pure speed win with identical arithmetic.`
...
- `2026-08-14 01:20` Iteration 9: GaussianMixture ported, and the FIRST src/ml/ module. Built the EM fit as a buffer-level API (row-major n x dim, no FcData) because it is the first kernel with two real consumers -- this Method and the LearnDistribution learner -- and find_clusters.c exports exactly one symbol, so a 56th static function would have forced the distribution learner to re-implement EM. Three non-mechanical generalisations: variance floor as a RIDGE not a clamp (a clamp needs an eigendecomposition per component per iteration); E-step in LOG space via log-sum-exp (a product of dim Gaussian factors underflows, and the 1-D kernel's response to a zero total was to give the point UNIFORM responsibilities -- wrong precisely for the informative outliers); diagonal fallback when a covariance will not factorise. Cholesky in-house rather than mat_lapack_dpotrf: dim is a feature count and the cost is the O(n k dim^2) E-step. UNIFIED (2nd after DBSCAN) -- pins and list_tests both unchanged, so the 1-D body and fc_gmm_fit were deleted. Full covariance needs > dim points per component, so k_max is capped at n/(dim+1) and the 9-point 5-D probe correctly gives ONE cluster; the blob test uses 24/blob. Also hit and fixed my own over-broad deletion: removing the 1-D body by text range also took fc_gmm_ndim, which -Wimplicit-function-declaration caught.
- `2026-08-14 01:05` Iteration 8: Spectral ported, and the plan was wrong in an instructive way. It said to replace the cut-along-sorted-order rule with k-means on the embedding; in fact the cut rule is a statement about the EMBEDDING, which is scalar in any dimension, so it ported unchanged by sorting on embedding value -- no k-means needed. The real work was elsewhere: (1) a disconnected affinity graph has a c-dimensional null space, so ONE Fiedler vector cannot separate c components, and this is the EASY case since exp(-(60/1.5)^2/2) is zero in doubles -- fixed by reading components with union-find first, merging nearest-first off the spanning tree when UpTo asks for fewer; (2) the natural-count threshold must use the MEAN jump, not the median, because a good embedding collapses within-cluster distance and drives the median to 0 -- two blobs were coming back as four clusters; (3) fixed a SEGFAULT for n<3 point input, which fell through to fc_scatter and indexed a NULL d->order. Two paths kept: the mean/median asymmetry is exactly reversed on raw data gaps. I also formed and REFUTED a wrong hypothesis mid-way (that one vector inherently cannot do 3 blobs) -- UpTo[3] being perfect was the clue that the embedding was fine and only the count was wrong; the single-vector limit turned out to be real but only for DISCONNECTED graphs.
