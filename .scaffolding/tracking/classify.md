...
- [x] `Method -> "LogisticRegression"` | `depends on: vocabulary` | `done (iteration 22)`
- [x] `DecisionTree / RandomForest` | `independent` | `DEFERRED, recorded not done. They reuse NOTHING built across five families: no distance, no density, no label-index arithmetic, no linear algebra. A tree needs an impurity criterion, a recursive splitter, a stopping rule and its own node representation in the model payload; a forest needs bootstrap resampling and feature subsampling on top. That is a family-sized piece of work, not an iteration, and doing it badly to claim coverage would be worse than saying so. The same judgement ContingencyTable got in family 4.`
- [x] `ContingencyTable (inherited from family 4)` | `depends on: vocabulary` | `DEFERRED, recorded not done. Now unblocked -- src/ml/encode.h is exactly the encoder it needs -- but it is a cross-tabulation utility rather than a learning method, so it sits outside the five ML families this work was scoped to. Cheap to add later on top of the vocabulary; listed as the obvious next follow-up.`
...
- `2026-08-14 03:55` Iteration 22: LogisticRegression, and FAMILY 5 IS CLOSED -- so is the whole five-family goal. IRLS with a small ridge on the non-intercept coefficients; the ridge is load-bearing rather than cosmetic, because on linearly separable data the unpenalised likelihood is UNBOUNDED and plain Newton diverges. Same shape of problem as the mixture's unbounded likelihood, same honest treatment: penalise, cap the iterations, and say so. Verified by an EXACT identity instead of an accuracy figure -- the probability at the fitted boundary (-intercept/coef) must be exactly 0.5, which ties the fit and the application together, and the boundary lands at exactly 5.0 on data with a gap from 3 to 7, with finite coefficients {-33.27, 6.65}. More than two classes DECLINES rather than silently binarising. DecisionTree/RandomForest and ContingencyTable both DEFERRED with reasons recorded rather than done badly.
