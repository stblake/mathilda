...
| C.8 | ~~**The integer band, again.** `Mod`/`Quotient` have registered binary kernels that decline `int64`~~ **DONE 2026-08-01** | `Union[kv]` 807 ms → **4.58 ms**, `Tally[jv]` 68.1 ms → **1.89 ms**, `DeleteDuplicates[jv]` 67.1 ms → **1.80 ms** | The measured cost was never on `Mod`. `Mod` **destroyed the packing** — its kernel computes in `double`, so it was rightly off `INT64_OK`, the gate materialised, and `Mod[Range[10^6] 7919, 1000]` answered with 10⁶ boxed Integers that every consumer then walked. `NDBinaryKernel` gained the exact-integer arms `NDUnaryKernel` has had since the narrowing kernels; `Mod` takes `to_int_i` only (its exactness follows the argument), `Quotient` takes both (it narrows to an exact Integer whatever it is given). See performance.md §15. `BitAnd`/`BitOr`/`BitXor`/`BitShiftLeft` still do not exist at all (C.12) |
