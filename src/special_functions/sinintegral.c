...
static bool si_machine_complex(double complex z, double complex* out, double* peak_out) {
    double peak_ = 0.0;
...
/* SinIntegral in the shared machine-kernel ABI, for the Compile[] engine and the NDArray
...
bool sinintegral_machine_complex(double are, double aim, double* ore, double* oim) {
    ...
    if (!si_machine_complex(are + aim * I, &r, &peak)) return false;
    ...
    /* 3. Numeric complex (Complex[..] with an inexact part). */
    {
        ...
        if (is_complex(arg, &re, &im) && (si_is_inexact(re) || si_is_inexact(im))) {
            ...
            if (si_to_double(re, &rr) && si_to_double(im, &ii)) {
                double complex v;
                if (si_machine_complex(rr + ii * I, &v, NULL)) {
