...
/* Collect the distinct generated parameters C[k] appearing in `e`. */
...
static Expr* reformat_solutions(const Expr* sols, const Expr* dom) {
    ...
    for (size_t i = 0; i < nt; i++) {
        ...
        Expr* conj = tuple_to_conj(sols->data.function.args[i], dom, &all_values);
...
Expr* reduce_integers(const Expr* expr, const Expr* vars_expr, const RForm* F,
    ...
    /* Reuse the Solve integer/rational engine. */
    ...
    if (nv == 1 && dom->type == EXPR_SYMBOL && dom->data.symbol.name == SYM_Integers)
        return reduce_univar_integers(F, vars[0], vars, nv);
    return NULL;
}
