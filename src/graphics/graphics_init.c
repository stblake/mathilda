...
void graphics_init(void) {
    ...
    symtab_set_docstring("ComplexPlot",
        ...
        "\tnumber.  Each cell's hue encodes Arg(f(z)) on the cyclic \"Cyclic\"\n"
        "\tramp, and |f(z)| sets an HSL lightness: zeros fade to black, poles to\n"
        "\twhite.  ComplexPlot is HoldAll: f and the iterator spec are held\n"
        "\tunevaluated until z is given a numeric complex value.\n"
        "\n"
        "\tOptions:\n"
        "\t  PlotPoints          grid resolution per axis (default 400)\n"
        "\t  ColorFunction       a named ramp string, or a function of the eight\n"
        "\t                      Mathematica arguments Re[z], Im[z], Abs[z],\n"
        "\t                      Arg[z], Re[f], Im[f], Abs[f], Arg[f] (so #8 is\n"
        "\t                      the phase of the value).  Named ramps:\n"
        "\t                      \"PhaseRings\" (hue=phase, brightness=log|w|\n"
        "\t                      rings — highlights poles and zeros), \"Cyclic\",\n"
        "\t                      \"Rainbow\", \"CoolTones\", \"WarmTones\",\n"
        "\t                      \"Greyscale\", \"Temperature\"\n"
        "\t  ColorFunctionScaling True (default): scale each of the eight\n"
        "\t                       arguments to [0,1] across the sampled domain\n"
        "\t                       before calling a custom ColorFunction\n"
        "\t  RegionFunction      f[x,y] mask; excluded cells are not drawn\n"
    ...
    symtab_set_docstring("ComplexPlot3D",
        ...
        "\t  ColorFunction       a named ramp string, or a function of the eight\n"
        "\t                      arguments Re[z], Im[z], Abs[z], Arg[z], Re[f],\n"
        "\t                      Im[f], Abs[f], Arg[f] (see ComplexPlot);\n"
        "\t                      \"PhaseRings\" recommended\n"
        "\t  ColorFunctionScaling True (default): scale the eight args to [0,1]\n"
        "\t  RegionFunction      f[x,y] mask\n"
        "\t  PlotLegends         Automatic / True: attach a vertical phase color\n"
