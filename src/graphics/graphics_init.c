...
void graphics_init(void) {
    ...
    symtab_set_docstring("Plot",
        ...
        "PlotTheme (\"Minimal\": no tick marks/numeric labels, lighter axes), "
    ...
    symtab_set_docstring("ParametricPlot3D",
        ...
        "3D surface patch. A single explicit surface with no PlotStyle/ "
        "ColorFunction given defaults to a \"Plasma\" height-based gradient, "
        "matching Plot3D's own default. Options: "
        ...
        "ColorFunction (a named ramp string -- \"Rainbow\", \"Temperature\"/"
        "\"Thermal\"/\"Plasma\", \"CoolTones\", \"WarmTones\", \"Viridis\", "
        "\"GreenYellow\", \"Greyscale\" -- or f[x,y,z] receiving scaled spatial "
        "coords, or f[x,z] / f[z] for height-based coloring), "
        "ColorFunctionScaling (default True), "
        "RegionFunction (f[x,y,z] mask; falls back to f[x,y] forms), "
        "PlotStyle, Axes, PlotTheme (\"Minimal\": quieter box axes, see Plot3D), "
        "PlotRange, AxesLabel, PlotLabel, Background, ImageSize "
    ...
    symtab_set_docstring("ContourPlot",
        ...
        "\t                     \"CoolTones\", \"WarmTones\", \"Greyscale\", \"Plasma\",\n"
        "\t                     \"Viridis\", \"GreenYellow\".\n"
        ...
        "\t  PlotPoints       - Initial grid resolution per axis (default 25); increase\n"
        "\t                     for smoother contours. MaxRecursion may refine beyond it.\n"
        "\t  MaxRecursion     - Doubles the whole grid's resolution (capped at 200\n"
        "\t                     points/axis) while a bilinear flatness spot-check fails,\n"
        "\t                     a global crack-free analogue of Plot3D's own\n"
    ...
    symtab_set_docstring("DensityPlot",
        ...
        "\t                      \"Greyscale\", \"Temperature\", \"Plasma\", \"Viridis\",\n"
        "\t                      \"GreenYellow\" (all keyed to normalised z value,\n"
        "\t                      t∈[0,1])\n"
    ...
    symtab_set_docstring("ArrayPlot",
        ...
        "\t                      \"WarmTones\", \"Plasma\", \"Viridis\", \"GreenYellow\",\n"
        "\t                      all keyed to the normalised entry value. Only\n"
    ...
    symtab_set_docstring("StreamPlot",
        ...
        "\t                   \"WarmTones\", \"Greyscale\", \"Temperature\", \"Plasma\",\n"
        "\t                   \"Viridis\", \"GreenYellow\"\n"
    ...
    symtab_set_docstring("Plot3D",
        ...
        "returns it as a Graphics3D[...] object. A single explicit surface "
        "with no PlotStyle/ColorFunction given defaults to a \"Plasma\" "
        "height-based gradient (dark blue-purple at the low points, through "
        "red/orange, to bright yellow at the high points) instead of a flat "
        "color. A list of functions Plot3D[{f1, f2, ...}, {x,...}, {y,...}] "
        "draws each surface in a distinct flat palette colour instead. "
        "Shares Plot's option semantics where "
        ...
        "function of scaled-x and z, or a named ramp string: \"Rainbow\", "
        "\"Temperature\"/\"Thermal\"/\"Plasma\", \"CoolTones\", \"WarmTones\", "
        "\"Viridis\", \"GreenYellow\", \"Greyscale\"), ColorFunctionScaling "
        "(default True), RegionFunction (f[x,y,z], or Plot's f[x,y]/f[x] "
        "forms), PlotRange (an explicit {zmin,zmax} z-band), Axes, "
        "PlotTheme (\"Minimal\": only the 3 near-corner box edges instead of "
        "the full wireframe cube, no numeric axis labels, lighter gray), "
        "PlotLabel, Background, ImageSize, "
        "Lighting (Automatic (default, Lambertian shading) or None to disable).");

    /* PlotTheme is an inert option keyword recognised by both renderers. */
    symtab_get_def("PlotTheme")->attributes |= ATTR_PROTECTED;
    symtab_set_docstring("PlotTheme",
        "PlotTheme\n\tA Plot / Plot3D / ListPlot / ParametricPlot / "
        "ParametricPlot3D / DensityPlot / ContourPlot option (also Graphics / "
        "Graphics3D directly) controlling axis chrome. "
        "PlotTheme -> \"Minimal\": quieter axes -- 2D drops tick marks and "
        "numeric tick labels (just the plain axis lines/frame box, in a "
        "lighter gray); 3D draws only the 3 box edges meeting at one corner "
        "instead of the full 12-edge wireframe cube, with no numeric axis "
        "labels. Any value other than \"Minimal\" (the default, unset) keeps "
        "today's full axes/box/labels.");
}
