...
/* ======================================================================== *
 *  Unified field-structure classifier + dispatcher (rat_canon_*)
 *
 *  Rationale.  Together/Cancel historically grew ~8 terminal paths, tried in
 *  sequence with overlapping type/size gates, running extension_autodetect up
 *  to three times and re-deciding kernel independence in several places.  The
 *  FLINT normalisation *engines* those paths call are correct and fast; the
 *  accumulated cost and the hang/SIGSEGV guards live in the ROUTING.
 *
 *  These functions classify the input's differential-field structure in ONE
 *  pass over its generator atoms (collect_variables), then dispatch to the one
 *  correct existing engine.  Independence of the generators is decided up front
 *  (Risch structure theorem, Bronstein §9.3: after km's log/exp canonicalisation
 *  the remaining Log/inverse-trig kernels are independent, exponentials are
 *  reduced to a common fundamental, algebraic constants form a number field,
 *  and a symbolic-radicand radical is one algebraic-function generator), so the
 *  weak classical multivariate GCD is never reached on dependent generators and
 *  the after-the-fact dependent-generator bails become unnecessary.
 *
 *  Currently wired in SHADOW mode only (MATHILDA_RATCANON=shadow): the result is
 *  compared against the existing cascade and divergences are logged, so parity
 *  can be proven before the builtins are switched over (plan Phase 2).
 * ======================================================================== */

typedef enum { RC_TOGETHER = 0, RC_CANCEL = 1 } RatCanonMode;

/* Forward decls for FLINT wrappers defined later in this file. */
static Expr* flint_gaussian_together(const Expr* arg);

/* Field-structure regime, decided once from the generator-atom profile. */
typedef enum {
    RCF_PLAIN_Q,        /* free symbols + integer powers only                  */
    RCF_TRANSCENDENTAL, /* + Log/Exp/inverse-trig kernels km handles (no alg)  */
    RCF_ALG_CONST,      /* + algebraic-number constant(s): I, Sqrt[c], zeta_n  */
    RCF_ALG_FUNC,       /* + one symbolic-radicand radical Sqrt[poly]/poly^p/q */
    RCF_DECLINE         /* dependent trig over alg const / mixed / unsupported */
} RatCanonRegime;

/* Generator profile accumulated by a direct walk of the expression.
 *
 * We do NOT use collect_variables here: it is tuned for polynomial variables
 * and recurses INTO radicals (surfacing `k` from `Sqrt[k]`) and drops constant
 * radicals (`Sqrt[2]`), so it never exposes the algebraic generators the regime
 * decision turns on.  rc_profile_walk instead treats each radical / kernel as
 * one opaque atom. */
typedef struct {
    bool has_alg_const;    /* I, Sqrt[rational], (-1)^(p/q), c^(p/q) (c rational) */
    bool has_alg_func;     /* Sqrt[symbolic] / symbolic^(p/q) (symbolic radicand) */
    bool has_exp;          /* Power[E,*] or Power[base, symbolic-exponent]         */
    bool has_indep_kernel; /* Log[*], ArcSin..ArcCsch (km-independent)             */
    bool has_trig;         /* Sin/Cos/Tan/Cot/Sec/Csc + hyperbolic (km declines)  */
    bool has_other_kernel; /* any other opaque function atom                       */
    bool has_free_sym;     /* at least one free-symbol generator                   */
    /* distinct symbolic-radicand radical atoms (borrowed, expr_eq-deduped) */
    Expr** radicals; size_t n_rad, cap_rad;
} RatAtomProfile;

/* True iff `h` names a km-independent inverse-trig/inverse-hyperbolic kernel. */
static bool rc_is_indep_inverse_head(const char* h) {
    static const char* const ks[] = {
...
/* Record a distinct symbolic-radicand radical atom (for the "exactly one"
 * ALG_FUNC gate). */
static void rc_add_radical(RatAtomProfile* p, Expr* v) {
    for (size_t i = 0; i < p->n_rad; i++)
        if (expr_eq(p->radicals[i], v)) return;
    if (p->n_rad == p->cap_rad) {
        p->cap_rad = p->cap_rad ? p->cap_rad * 2 : 4;
        p->radicals = realloc(p->radicals, p->cap_rad * sizeof(Expr*));
    }
    p->radicals[p->n_rad++] = v;
}

/* True if `e` contains an actual denominator anywhere — a Power[base, k] with
 * k a negative integer or negative-numerator rational.  Used to detect a
 * kernel argument (a Sqrt radicand, a Log/inverse-trig argument) that itself
 * carries a nested fraction: the FLINT normalizers treat the kernel as an
 * opaque atom and do NOT combine inside it, whereas the classical
 * together_recursive descends and combines — so such inputs are declined to
 * keep the more-simplified classical form. */
static bool rc_arg_has_denom(Expr* e) {
    if (!e || e->type != EXPR_FUNCTION) return false;
    if (e->data.function.head && e->data.function.head->type == EXPR_SYMBOL &&
        ...
        if (is_rational(ex, &pn, &qn) && pn < 0) return true;
    }
    if (rc_arg_has_denom(e->data.function.head)) return true;
    for (size_t i = 0; i < e->data.function.arg_count; i++)
        if (rc_arg_has_denom(e->data.function.args[i])) return true;
    return false;
}

/* Direct recursive generator-profiling walk.  Recurses through Plus/Times and
 * integer/negative-integer Power (the rational-function skeleton); every radical
 * / transcendental kernel is one opaque atom (its argument is not descended into
 * for classification). */
static void rc_profile_walk(Expr* e, RatAtomProfile* p) {
    if (!e) return;
    switch (e->type) {
        case EXPR_INTEGER: case EXPR_BIGINT: case EXPR_REAL:
            return;
        case EXPR_SYMBOL:
            if (e->data.symbol.name == SYM_I) p->has_alg_const = true;
            else if (e->data.symbol.name == SYM_E) p->has_exp = true;
            else if (rat_is_numeric_const(e)) return;   /* Pi, EulerGamma, ... */
            /* trigrat/poly radical-substitution placeholders ($pc_rad_N$,
             * $pc_radgen N$) are NOT free variables: they carry an implicit
             * algebraic relation (rad^q = g) that the substituting pass reduces
             * modulo afterwards.  A field-structure classifier cannot model that,
             * so decline — keep such internal-integrator state on the classical
             * path (as today). */
            else if (strncmp(e->data.symbol.name, "$pc_rad", 7) == 0)
                p->has_other_kernel = true;
            else p->has_free_sym = true;
            return;
        case EXPR_FUNCTION: break;
        default: p->has_other_kernel = true; return;
    }
    if (!e->data.function.head || e->data.function.head->type != EXPR_SYMBOL) {
        p->has_other_kernel = true; return;
    }
    const char* h = e->data.function.head->data.symbol.name;
    size_t ac = e->data.function.arg_count;
    Expr** av = e->data.function.args;

    if (h == SYM_Complex) { p->has_alg_const = true; return; }
    if ((h == SYM_Plus || h == SYM_Times) ) {
        for (size_t i = 0; i < ac; i++) rc_profile_walk(av[i], p);
        return;
    }
    if (h == SYM_Sqrt && ac == 1) {
        Expr* r = av[0];
        int64_t pp, qq;
        if (r->type == EXPR_INTEGER || r->type == EXPR_BIGINT || is_rational(r, &pp, &qq))
            p->has_alg_const = true;
        else if (rc_arg_has_denom(r)) p->has_other_kernel = true;  /* radicand needs Together */
        else { p->has_alg_func = true; rc_add_radical(p, e); }
        return;
    }
    if (h == SYM_Power && ac == 2) {
        Expr* base = av[0];
        ...
        if (base->type == EXPR_SYMBOL && base->data.symbol.name == SYM_E) {
            if (rc_arg_has_denom(exp)) p->has_other_kernel = true;  /* exponent needs Together */
            else p->has_exp = true;
            return;
        }
        int64_t pp, qq, bp, bq;
        if (is_rational(exp, &pp, &qq) && qq != 1) {
            if (base->type == EXPR_INTEGER || base->type == EXPR_BIGINT ||
                is_rational(base, &bp, &bq) ||
                (base->type == EXPR_SYMBOL && base->data.symbol.name == SYM_I))
                p->has_alg_const = true;
            else if (rc_arg_has_denom(base)) p->has_other_kernel = true;
            else { p->has_alg_func = true; rc_add_radical(p, e); }
            return;
        }
        if (rat_exp_is_symbolic(exp)) {
            if (rc_arg_has_denom(exp) || rc_arg_has_denom(base)) p->has_other_kernel = true;
            else p->has_exp = true;
            return;
        }
        /* Integer exponent (incl. negative denominator power): descend base. */
        rc_profile_walk(base, p);
        return;
    }
    if (h == SYM_Log || rc_is_indep_inverse_head(h)) {
        if (ac == 1 && rc_arg_has_denom(av[0])) p->has_other_kernel = true;
        else p->has_indep_kernel = true;
        return;
    }
    if (rat_is_trig_kernel(e)) { p->has_trig = true; return; }
    p->has_other_kernel = true;
}

/* Decide the field-structure regime of `arg` in one pass. */
static RatCanonRegime rat_canon_classify(Expr* arg) {
    RatAtomProfile p; memset(&p, 0, sizeof p);
    rc_profile_walk(arg, &p);
    int n_alg_func = (int)p.n_rad;
    free(p.radicals);

    RatCanonRegime reg;
    if (p.has_other_kernel) reg = RCF_DECLINE;               /* km can't atomise */
    else if (p.has_alg_func) {
        /* One clean symbolic-radicand radical, nothing else algebraic/trig. */
        if (n_alg_func == 1 && !p.has_alg_const && !p.has_trig)
            reg = RCF_ALG_FUNC;
        else reg = RCF_DECLINE;                              /* multi-radical / mixed */
    }
    else if (p.has_alg_const) {
        if (p.has_trig) reg = RCF_DECLINE;                   /* dependent trig over K */
        else reg = RCF_ALG_CONST;
    }
    else if (p.has_trig) reg = RCF_DECLINE;                  /* pure trig: classical */
    else if (p.has_exp || p.has_indep_kernel) reg = RCF_TRANSCENDENTAL;
    else reg = RCF_PLAIN_Q;
    return reg;
}

/* Phase-2 switch gate.  Default ON — the unified dispatch is the primary path.
 * MATHILDA_RATCANON=off disables it (pure classical cascade, for A/B); =shadow
 * also disables it so the shadow harness compares the dispatch against the
 * cascade's output rather than against itself. */
static bool rat_canon_enabled(void) {
    static int en = -1;
    if (en < 0) {
        const char* e = getenv("MATHILDA_RATCANON");
        en = (e && (strcmp(e, "off") == 0 || strcmp(e, "shadow") == 0)) ? 0 : 1;
    }
    return en;
}

/* Match the classical denominator-sign convention (leading coefficient
 * positive).  The FLINT normalizers can emit -(N)/(-D) where the classical path
 * emits N/D — e.g. Together[1/(x-I)+1/(x+I)] -> (2x)/(1+x^2), not
 * -(2x)/(-1-x^2).  If the denominator's leading (highest-degree) term is
 * negative — in Mathilda's constant-first Plus ordering, the LAST summand — flip
 * the sign of numerator and denominator.  Leaves e.g. 1/(x-1) untouched (leading
 * term +x). */
/* Negate `e`, distributing over a top-level Plus so -(−1−x^2) becomes 1+x^2
 * rather than the evaluator's Times[-1, Plus[...]] (which folds the -1 straight
 * back out).  Consumes nothing; returns a fresh owned Expr. */
static Expr* rc_negate(Expr* e) {
    if (e->type == EXPR_FUNCTION && head_is(e, SYM_Plus) &&
        e->data.function.arg_count > 0) {
        size_t c = e->data.function.arg_count;
        Expr** args = malloc(sizeof(Expr*) * c);
        for (size_t i = 0; i < c; i++) args[i] = negate_expr(e->data.function.args[i]);
        Expr* p = eval_and_free(expr_new_function(expr_new_symbol(SYM_Plus), args, c));
        free(args);
        return p;
    }
    return negate_expr(e);
}

static Expr* rc_sign_normalize(Expr* r) {
    if (!r) return r;
    Expr* n; Expr* d;
    extract_num_den(r, &n, &d);
    /* Flip only the FLINT -(N)/(-D) artifact: a denominator all of whose terms
     * are negative, so -1 factors out cleanly.  Never a genuinely mixed-sign
     * polynomial (x - 1, a^2 - a b), where either surface form is equally
     * canonical and the classical path's choice must be preserved. */
    bool flip;
    if (d->type == EXPR_FUNCTION && head_is(d, SYM_Plus) &&
        ...
        flip = is_superficially_negative(d);
    }
...
/* Dispatch `arg` (already option-stripped) to the single engine its regime
 * selects.  Returns a fresh reduced Expr (sign-normalised to the classical
 * convention), or NULL to decline (caller keeps the classical path).  `mode`
 * selects Together (combine) vs Cancel read-back. */
static Expr* rat_canon_dispatch(Expr* arg, RatCanonMode mode) {
#ifdef USE_FLINT
    Expr* r = NULL;
    switch (rat_canon_classify(arg)) {
        case RCF_PLAIN_Q:
        case RCF_TRANSCENDENTAL:
            r = (mode == RC_CANCEL) ? flint_rational_cancel(arg)
                                    : flint_rational_together(arg);
            break;
        case RCF_ALG_CONST:
            /* Number field Q(I, Sqrt[c], zeta_n): rigorous GCD+divide first
             * (flint_cancel_fraction), then the Gaussian residual path.  We do
             * NOT rationalise radicals out of the denominator here — Together/
             * Cancel are WL-faithful (1/(x-Sqrt[2]) stays), so the norm-based
             * flint_algebraic_field_canonical (the RootReduce engine) is out of
             * scope; declining hands such inputs to the classical extension
             * path exactly as the old cascade did. */
            r = flint_cancel_fraction(arg);
            if (!r) r = (mode == RC_CANCEL) ? flint_gaussian_cancel(arg)
                                            : flint_gaussian_together(arg);
            break;
        case RCF_ALG_FUNC:
            /* Q(params)(Sqrt[k]) symbolic radicand.  General extension GCD+divide
             * first (flint_cancel_fraction routes to flint_parametric_sqrt_gcd),
             * then the whole-rational fmpz_mpoly_q normaliser (sqrt k -> S,
             * k -> S^2) — the correct home for the input that currently
             * mis-routes to Phase E's multivariate PolynomialGCD.
             *
             * We deliberately do NOT fall through to flint_algebraic_field_together
             * here: it is WL-faithful "combine" but too aggressive for Cancel/
             * Together — it expands radicands (Sqrt[1/2(x-y)] -> Sqrt[1/2 x-1/2 y]),
             * expands squares of radicals, and does not perform the cube-root
             * relation cancellation ((y-1)/(y^(1/3)-1) -> 1+y^(1/3)+y^(2/3)) that
             * the classical radical-substitution pass does.  So anything the two
             * FLINT engines below decline is handed to the classical path. */
            r = flint_cancel_fraction(arg);
            if (!r) r = flint_parametric_field_normalize(arg);
            break;
        case RCF_DECLINE:
        default:
            return NULL;
    }
    return rc_sign_normalize(r);
#else
    (void)arg; (void)mode;
    return NULL;
#endif
...
/* Shadow-parity check (MATHILDA_RATCANON=shadow).  Compares the unified
 * dispatcher's result for `res` against the cascade's `old_result`.  Prints only
 * DIVERGENCE lines immediately (with both forms) plus a one-shot summary at
 * exit; never changes what the builtin returns.  A reentrancy guard keeps the
 * dispatcher's / evaluator's own internal Together/Cancel calls from recursing
 * back into the check. */
static long rc_sh_ok = 0, rc_sh_declined = 0, rc_sh_diverged = 0, rc_sh_formdiff = 0;
static int  rc_sh_active = 0, rc_sh_atexit = 0;

static void rat_canon_shadow_summary(void) {
    fprintf(stderr, "[ratcanon] SUMMARY covered=%ld declined=%ld DIVERGED=%ld formdiff=%ld\n",
            rc_sh_ok, rc_sh_declined, rc_sh_diverged, rc_sh_formdiff);
}

/* Exact rational-function equality: a == b iff the numerator of Together[a - b]
 * is identically zero.  Robust to the cosmetic sign/leading-coefficient
 * normalisation differences between the FLINT and classical read-back paths
 * (e.g. (-1-t^4)/(-1+t^4) vs (1+t^4)/(1-t^4)), which expr_eq would flag. */
static bool rc_math_equal(Expr* a, Expr* b) {
    if (expr_eq(a, b)) return true;
    ...
    Expr* diff = expr_new_function(expr_new_symbol(SYM_Plus),
                                   (Expr*[]){expr_copy(a), nb}, 2);
    Expr* tg = expr_new_function(expr_new_symbol(SYM_Together), (Expr*[]){diff}, 1);
    Expr* r = evaluate(tg);   /* owns tg */
    bool zero = r && ((r->type == EXPR_INTEGER && r->data.integer == 0) ||
                      (r->type == EXPR_REAL && r->data.real == 0.0));
    if (!zero && r) {
        /* Together can't collapse cyclotomic disguised-zeros like
         * 1 - (-1)^(1/3) + (-1)^(2/3) == 0; RootReduce (qqbar) can. */
        Expr* rr = expr_new_function(expr_new_symbol("RootReduce"),
                                     (Expr*[]){expr_copy(r)}, 1);
        Expr* rv = evaluate(rr);
        zero = rv && ((rv->type == EXPR_INTEGER && rv->data.integer == 0) ||
                      (rv->type == EXPR_REAL && rv->data.real == 0.0));
        if (rv) expr_free(rv);
    }
    if (r) expr_free(r);
    return zero;
...
static void rat_canon_shadow(Expr* res, Expr* old_result, RatCanonMode mode) {
    /* Cache the env probe once — Simplify issues thousands of Together/Cancel
     * calls and getenv-per-call would add measurable overhead. */
    static int rc_sh_enabled = -1;
    if (rc_sh_enabled < 0) {
        const char* env = getenv("MATHILDA_RATCANON");
        rc_sh_enabled = (env && strcmp(env, "shadow") == 0) ? 1 : 0;
    }
    if (!rc_sh_enabled) return;
    if (rc_sh_active) return;                     /* no reentrancy */
    if (res->type != EXPR_FUNCTION || res->data.function.arg_count < 1) return;
    size_t argc = res->data.function.arg_count;
    bool auto_flag = false;
    (void)extract_extension_option_full(res, &argc, &auto_flag);
    if (argc != 1) return;
    Expr* arg = res->data.function.args[0];
    const char* tag = (mode == RC_CANCEL) ? "Cancel" : "Together";

    if (!rc_sh_atexit) { rc_sh_atexit = 1; atexit(rat_canon_shadow_summary); }

    rc_sh_active = 1;
    Expr* newr = rat_canon_dispatch(arg, mode);
    if (!newr) { rc_sh_declined++; rc_sh_active = 0; return; }
    if (!old_result) { rc_sh_declined++; expr_free(newr); rc_sh_active = 0; return; }

    Expr* a = evaluate(expr_copy(newr));
    Expr* b = evaluate(expr_copy(old_result));
    if (!rc_math_equal(a, b)) {
        rc_sh_diverged++;
        char* sa = expr_to_string(a);
        char* sb = expr_to_string(b);
        char* si = expr_to_string(arg);
        fprintf(stderr, "[ratcanon] %s DIVERGENCE in=%s new=%s old=%s\n",
                tag, si ? si : "?", sa ? sa : "?", sb ? sb : "?");
        free(sa); free(sb); free(si);
    } else {
        rc_sh_ok++;
        /* Math-equal but different surface FORM: switching the builtin to the
         * dispatcher (Phase 2) would change this call's user-visible output.
         * Count it so the Phase-2 form-change surface is measured up front. */
        if (!expr_eq(a, b)) {
            rc_sh_formdiff++;
            if (getenv("MATHILDA_RATCANON_FORMS")) {
                char* sa = expr_to_string(a); char* sb = expr_to_string(b);
                char* si = expr_to_string(arg);
                fprintf(stderr, "[ratcanon] %s FORMDIFF in=%s new=%s old=%s\n",
                        tag, si?si:"?", sa?sa:"?", sb?sb:"?");
                free(sa); free(sb); free(si);
            }
        }
    }
    expr_free(a); expr_free(b); expr_free(newr);
    rc_sh_active = 0;
}
...
Expr* builtin_cancel(Expr* res) {
    ...
    rat_canon_shadow(res, out, RC_CANCEL);
...
Expr* builtin_together(Expr* res) {
    ...
    rat_canon_shadow(res, out, RC_TOGETHER);
    if (out && memo) factor_memo_store(memo, res, out);
    return out;
}
