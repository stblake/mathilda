...
static bool ns_apply_option(Expr* rule, NsOpts* o) {
    ...
    if (name == SYM_NSumTerms) {
        if (rhs->type == EXPR_INTEGER && rhs->data.integer >= 0) {
            o->nsum_terms = (int)rhs->data.integer; return true;
        }
 ...
 *  with N = imin + (NSumTerms)·di and the head terms summed exactly.  *
 *  The tail integral uses exp-sinh quadrature; the derivative          *
 *  corrections use iterated symbolic D and are truncated at the        *
 *  smallest term (the series is asymptotic).                           *
 * ------------------------------------------------------------------ */

#define NS_EM_MAXJ 12          /* cap on correction terms (derivative order 2J-1) */
#define NS_EM_MAX_NODES 1200   /* stop corrections if a derivative grows past this */

/* Bounded node count: returns min(size, limit+1) without walking a huge tree.
 * Iterated symbolic derivatives of summands like 1/Fibonacci[x] blow up, so the
 * Euler–Maclaurin correction loop uses this to stop before the cost explodes. */
...
static Expr* ns_em_machine(NsCtx* c, const char* var, NsOpts* o) {
    int m = o->nsum_terms < 0 ? 0 : o->nsum_terms;
    double _Complex H = 0.0;
    for (int k = 0; k < m; k++) {
        double _Complex t;
    ...
    /* Corrections: symbolic derivatives require the index var to be unbound. */
    ...
    double dipow = ddi;                         /* di^1, then ·= di^2 each step */
    bool failed = false, toobig = false;
    for (int j = 1; j <= NS_EM_MAXJ; j++) {
        int ord = 2 * j - 1;
        while (dcur && cur < ord) {
        ...
        if (!dcur || toobig) break;     /* keep the corrections already summed */
        double coeff = ns_em_coeff_machine(j);
        double _Complex dval;
        bool ok = ns_eval_complex_machine(c, dcur, expr_copy(Nval), &dval);
        ns_bind_clear_temp(symtab_get_def(var));          /* free var for next D */
        if (!ok) { if (j == 1) failed = true; break; }
        double _Complex corr = coeff * dipow * dval;
    ...
    return ns_from_complex_d(H + tail);
}
...
/* B_{2j}/(2j)! as a real MPFR at `bits`. */
...
static Expr* ns_em_mpfr(NsCtx* c, const char* var, NsOpts* o) {
    long bits = o->bits;
    mpfr_prec_t p = (mpfr_prec_t)bits;
    int m = o->nsum_terms < 0 ? 0 : o->nsum_terms;

    mpfr_t Hr, Hi, tr, ti, tail_r, tail_i, Iv_r, Iv_i, fN_r, fN_i;
    mpfr_t ddi_m, dipow, coeff, dvr, dvi, cr, ci, Na;
    mpfr_inits2(p, Hr, Hi, tr, ti, tail_r, tail_i, Iv_r, Iv_i, fN_r, fN_i,
                ddi_m, dipow, coeff, dvr, dvi, cr, ci, Na, (mpfr_ptr)0);
    ...
    if (ok) {
        integ = dequad_halfline_mpfr(ns_sample_x_mpfr, c, Na, bits, 1e-13, 16, Iv_r, Iv_i, &abserr);
    ...
    if (ok) {
        ...
        double prev_mag = INFINITY;
        double goal = (o->prec_goal > 0 ? o->prec_goal : numeric_bits_to_digits(bits) - 2);
        double tol = pow(10.0, -goal) * (ns_l1_d(Hr, Hi) + ns_l1_d(tail_r, tail_i) + 1.0);
        bool toobig = false;
        for (int j = 1; j <= NS_EM_MAXJ; j++) {
            int ord = 2 * j - 1;
            while (dcur && cur < ord) {
            ...
            if (!dcur || toobig || !ns_em_coeff_mpfr(j, bits, coeff)) break;
            bool dok = ns_eval_complex_mpfr(c, dcur, expr_copy(Nval), dvr, dvi);
            ns_bind_clear_temp(symtab_get_def(var));
            if (!dok) break;
            mpfr_mul(cr, coeff, dipow, MPFR_RNDN);   /* cr = coeff·di^ord       */
            mpfr_mul(ci, cr, dvi, MPFR_RNDN);        /* ci = corr imag          */
            mpfr_mul(cr, cr, dvr, MPFR_RNDN);        /* cr = corr real          */
            double mag = ns_l1_d(cr, ci);
            if (j > 1 && mag > prev_mag) break;
...
static int ns_cvz_terms(const NsOpts* o) {
    double bits = o->prec_mpfr ? (double)o->bits : 53.0;
...
/* Ratio test for divergence (consistently |a_{k+1}/a_k| > 1 in the tail).
 * Deliberately blind to ratios → 1 (e.g. the harmonic series), matching NSum. */
static bool ns_diverges(const NsProbe* pr) {
    if (pr->n < 8) return false;
    int bad = 0, tot = 0;
    for (int k = pr->n - 5; k < pr->n - 1; k++) {
        if (k < 1) continue;
        double a = cabs(pr->t[k]), b = cabs(pr->t[k + 1]);
        if (a <= 0.0) continue;
        tot++;
        if (b / a > 1.1) bad++;
    }
    return tot > 0 && bad == tot;
}

/* Resolve Automatic to a concrete method symbol from the term profile. */
static const char* ns_choose_method(const NsProbe* pr) {
    if (!pr->ok || pr->n < 4) return SYM_WynnEpsilon;
    if (ns_is_alternating(pr) && ns_mag_decreasing(pr, 0)) return SYM_AlternatingSigns;
    if (ns_mag_decreasing(pr, pr->n / 2)) return SYM_EulerMaclaurin;   /* tail monotone */
...
static Expr* ns_sum_infinite(NsCtx* c, const char* var, NsOpts* o, bool do_verify) {
    ...
    if (do_verify || method == SYM_Automatic) {
        NsProbe pr = ns_probe(c, -1);
        if (do_verify && ns_diverges(&pr)) {
            ns_warn("div", "the sum does not appear to converge");
            return expr_new_symbol("ComplexInfinity");
        }
        if (method == SYM_Automatic) method = ns_choose_method(&pr);
    }
...
#ifdef USE_MPFR
        out = o->prec_mpfr ? ns_em_mpfr(c, var, o) : ns_em_machine(c, var, o);
#else
        out = ns_em_machine(c, var, o);
#endif
...
static Expr* ns_run_single(Expr* body, const char* var, Expr* imin, Expr* imax,
    ...
    if (infinite) {
        ...
        NsProbe pr = ns_probe(&ctx, -1);
        bool decays = pr.ok && ns_mag_decreasing(&pr, pr.n / 2) && !ns_diverges(&pr);
        if (decays) {
