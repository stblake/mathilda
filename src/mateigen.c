...
static bool arnoldi_automatic_prefers(Expr* k_spec, size_t n) {
...
static Expr* mateigen_build_complex_eigvec_list_rect(const double* V_re,
                                                       const double* V_im,
                                                       size_t k_vectors,
                                                       size_t n_components) {
...
static size_t matD_bandwidth_real(const double* A, size_t n, double tol) {
    if (n <= 1) return 0;
    size_t b = 0;
...
static size_t matD_bandwidth_complex(const MatD* A, double tol) {
    size_t n = A->n;
    if (n <= 1) return 0;
    size_t b = 0;
    for (size_t k = n - 1; k > 0; k--) {
...
static bool banded_automatic_prefers(Expr* m, int64_t n) {
    if (n <= 8) return false;
    MatD A;
...
static void direct_hessenberg_real_M(mpfr_t* A, size_t n, mpfr_prec_t bits,
                                       mpfr_t* u, mpfr_t* Q,
                                       mpfr_t* tmp /* >= 8 cells */) {
    (void)bits;
    mpfr_t* sigma  = &tmp[0];
...
static int direct_qr_real_general_M(mpfr_t* H, size_t n, mpfr_prec_t bits,
...
static void direct_sort_perm_desc_abs_complex_M(const mpfr_t* re,
                                                  const mpfr_t* im,
                                                  size_t n, size_t* perm) {
    if (n == 0) return;
...
static Expr* direct_build_complex_eigenvalue_list_M(const mpfr_t* re,
                                                      const mpfr_t* im,
                                                      size_t n,
                                                      const size_t* perm) {
    Expr** items = (Expr**)malloc(sizeof(Expr*) * n);
...
static void schur_compute_eigvecs_M(const mpfr_t* H, const mpfr_t* Q, size_t n,
                                      mpfr_prec_t bits,
...
static Expr* mateigen_build_complex_eigvec_list_rect_M(const mpfr_t* V_re,
                                                         const mpfr_t* V_im,
                                                         size_t k_vectors,
                                                         size_t n_components) {
    Expr** rows = (Expr**)malloc(sizeof(Expr*) * k_vectors);
...
static size_t matM_bandwidth_real(const mpfr_t* A, size_t n,
                                    const mpfr_t tol) {
    if (n <= 1) return 0;
    size_t b = 0;
    mpfr_t a;
...
static size_t matM_bandwidth_complex(const MatM* A, const mpfr_t tol) {
    size_t n = A->n;
    if (n <= 1) return 0;
    size_t b = 0;
    mpfr_t mag;
 ...
 * path. */
static Expr* arnoldi_dispatch(Expr* m, Expr* a, int64_t n,
                                MateigenWant want, Expr* k_spec,
                                Expr* method_value) {
    if (a != NULL) return NULL;          /* generalised: symbolic only */
    if (n <= 0)    return NULL;
...
static Expr* banded_dispatch(Expr* m, Expr* a, int64_t n,
                               MateigenWant want, Expr* k_spec,
                               Expr* method_value) {
    if (a != NULL) return NULL;
    if (n <= 0)    return NULL;
