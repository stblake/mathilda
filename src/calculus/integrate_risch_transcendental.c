...
static int rde_spde_field(Expr* a, Expr* b, Expr* c, RdeCtx* C, long n, RdeSpde* out) {
    out->b = out->c = out->alpha = out->beta = NULL; out->m = 0;
    if (n < 0) {
        if (rt_is_zero(c)) {
            out->b = expr_new_integer(0); out->c = expr_new_integer(0);
...
static Expr* rde_polyrischde_nocancel_field(Expr* b, Expr* c, RdeCtx* C, long n) {
    long db = rc_deg(b, C);
    Expr* lcb = rc_lc(b, C);
    Expr* q = expr_new_integer(0);
    Expr* cc = expr_copy(c);
...
static Expr* rde_tower(Expr* f, Expr* g, RdeCtx* C) {
    if (rt_is_zero(g)) return expr_new_integer(0);
