/* graphjoin.c - GraphJoin[g1, g2]: the graph join. The vertices are the disjoint
 * union of g1's and g2's, relabelled 1..n1+n2 (g1's block first), the edges are
 * those of g1 and g2 (relabelled) together with an undirected edge joining every
 * g1 vertex to every g2 vertex.
 *
 * Relabelling to consecutive integers gives a genuine disjoint union even when
 * the two graphs share vertex names. Within each block the original edge kind is
 * preserved; the n1*n2 cross edges are undirected. The result has
 * m1 + m2 + n1*n2 edges and is a canonical Graph. O((V+E) + n1*n2).
 *
 * K1 join K1 is a single edge; P2 join K1 is a triangle; K_m join K_n is
 * K_{m+n}. Memory (SPEC section 4): returns a freshly-built Graph; frees res.
 * NULL unless both arguments are valid graphs.
 */
...
Expr* builtin_graph_join(Expr* res) {
    ...
    size_t total = m1 + m2 + (size_t)n1 * (size_t)n2;
    Expr** ec = malloc((total > 0 ? total : 1) * sizeof(Expr*));
    size_t me = 0;
    ...
    for (int i = 0; i < n1; i++)
        for (int j = 0; j < n2; j++) {
            Expr* args[2] = { expr_new_integer(i + 1), expr_new_integer(n1 + j + 1) };
