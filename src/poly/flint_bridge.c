...
Expr* flint_tower_reduce(const Expr* frac,
    ...
    /* LEX variable order: eliminable radicand-variables FIRST, then generators,
     * then the remaining params/main var.  `elim_vars` are the radicand variables
     * that are a BARE SYMBOL (y in y^(1/3), b in Sqrt[b]); ordering such a var
     * above its generator makes the relation g^q - var a Groebner basis with
     * leading term `var`, so divrem_ideal eliminates it (y -> g^q) and the
     * re-canonicalise cancels ((y-1)/(y^(1/3)-1) -> 1+y^(1/3)+y^(2/3),
     * (a^2-b)/(a-Sqrt b) -> a+Sqrt b).  A CONSTANT radicand (Sqrt2) or a POLYNOMIAL
     * radicand (Sqrt[1+x^2]) is NOT eliminable — its generator leads naturally
     * (g^q -> const / sum-of-conjugates), so x^2 is never rewritten to g^2-1 and
     * (1+x^2)^(3/2) poly(x) is left intact. */
    VarSet fvars; memset(&fvars, 0, sizeof fvars);
    for (int i = 0; i < n_elim; i++) varset_add(&fvars, elim_vars[i]);   /* leading */
    for (int i = 0; i < n_alg; i++) varset_add(&fvars, alg_syms[i]);     /* generators */
    VarSet all; memset(&all, 0, sizeof all);
...
Expr* flint_tower_reduce(const Expr* frac, const char* const* alg_syms,
                         const Expr* const* relations, int n_alg,
                         const char* const* elim_vars, int n_elim) {
    (void)frac; (void)alg_syms; (void)relations; (void)n_alg;
    (void)elim_vars; (void)n_elim; return NULL;
}
Expr* flint_apart_over_q(const Expr* R, const Expr* const* bases,
                         const int64_t* ks, int m,
                         const Expr* C, const char* var) {
