...
/* `Inverse` lives in src/matinv.c (which also implements `PseudoInverse`).
 * See matinv.h for the API and matinv_init() for registration. */
...
/* Eigenvalues and Eigenvectors live in src/mateigen.c. */
...
void linalg_init(void) {
    ...
    /* Inverse and PseudoInverse live in src/matinv.c (matinv_init). */
    symtab_add_builtin("MatrixPower", builtin_matrixpower);
    symtab_get_def("MatrixPower")->attributes |= ATTR_PROTECTED;
    /* Eigenvalues and Eigenvectors live in src/mateigen.c; registered via mateigen_init(). */
}
