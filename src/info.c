...
void info_init(void) {
    ...
    symtab_set_docstring("LeastSquares",
        ...
        "  \"IterativeRefinement\" — one refinement pass over Direct;\n"
        "                          reduces round-off in numeric systems\n"
        "                          (no-op for exact input)\n"
        "  \"LSQR\"                — Paige-Saunders LSQR; the method name\n"
        "                          is accepted by the parser and the\n"
        "                          current dispatch goes through Direct\n"
        "  \"Krylov\"              — iterative Krylov / CGNR method;\n"
        "                          currently dispatched to Direct\n"
        "\n"
