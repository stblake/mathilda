...
void info_init(void) {
    ...
    symtab_set_docstring("FindMinimum",
        ...
        "\t\"LBFGSB\"            limited-memory BFGS (history 10) with box "
        "constraints via active-set projection; O(m*n) per step, so it scales "
        "to large n where QuasiNewton's dense Hessian is O(n^2); aliases "
        "\"LBFGS\", \"LimitedMemoryBFGS\" (a Mathilda extension).\n"
        "\t\"Powell\"            derivative-free conjugate directions with "
        "Brent line search; no gradient, so it suits non-smooth or black-box "
        "objectives; supports box bounds (not general constraints); alias "
        "\"PrincipalAxis\".\n"
        "\t\"NelderMead\"        derivative-free downhill simplex; no gradient; "
        "robust on smooth black-box objectives (weak on non-smooth -- prefer "
        "\"Powell\" there); supports box bounds (not general constraints).\n"
        "\t\"TNC\"               Hessian-free truncated Newton: inner conjugate "
        "gradient solving the Newton system via Hessian-vector products (finite "
        "differences of the gradient), with active-set box bounds; O(n) memory, "
        "so it scales where \"Newton\" (full symbolic Hessian) cannot, using "
        "truer curvature than \"LBFGSB\"; general constraints via the penalty "
        "wrapper; alias \"TruncatedNewton\".\n"
        "\t\"SLSQP\"             sequential least-squares QP (Han-Powell SQP) "
        "for smooth constrained problems: each step solves a QP (BFGS "
        "Lagrangian-Hessian, linearized constraints) accepted by an L1-penalty "
        "line search; handles equality, inequality AND box constraints "
        "DIRECTLY (not via the penalty wrapper), so it reaches the true "
        "constrained optimum with super-linear local convergence; reduces to "
        "damped-BFGS when unconstrained; alias "
        "\"SequentialQuadraticProgramming\".\n"
        "\t\"COBYLA\"            Powell's derivative-free linear-approximation "
        "trust-region method: models f and every constraint by a linear "
        "approximation and solves a two-stage trust-region LP (feasibility then "
        "objective) with an L-infinity penalty merit; the only derivative-free "
        "method that accepts general (non-box) constraints, so it suits "
        "non-smooth / black-box CONSTRAINED objectives; equalities handled by "
        "splitting; machine precision only.\n"
        "\t\"COBYQA\"            derivative-free trust-region SQP with QUADRATIC "
        "interpolation models (vs COBYLA's linear): captures curvature, so it "
        "converges tighter on smooth problems and navigates curved valleys "
        "(Rosenbrock) that COBYLA cannot; handles equality + inequality + bound "
        "constraints natively; machine precision only.\n"
        "\t\"NewtonCG\"          line-search truncated Newton: inner CG solves "
        "the Newton system via Hessian-vector products with negative-curvature "
        "truncation, then a Wolfe line search; Hessian-free, scales to large n; "
        "UNCONSTRAINED (rejects constraints, ignores bounds); alias "
        "\"Newton-CG\".\n"
        "\t\"Dogleg\"            trust-region Powell dogleg: blends the Cauchy "
        "and Newton points on the dense Hessian along the dogleg path; falls "
        "back to steepest-to-boundary when the model is not positive definite; "
        "UNCONSTRAINED; alias \"dogleg\".\n"
        "\t\"TrustNCG\"          trust-region Steihaug-Toint truncated CG using "
        "Hessian-vector products only; Hessian-free; UNCONSTRAINED; aliases "
        "\"trust-ncg\", \"TrustRegionNewtonCG\".\n"
        "\t\"TrustExact\"        trust-region with a near-exact More-Sorensen "
        "subproblem on the dense Hessian (Levenberg shift + hard-case branch); "
        "handles indefinite Hessians dogleg cannot; UNCONSTRAINED; alias "
        "\"trust-exact\".\n"
        "\t\"TrustKrylov\"       trust-region GLTR: Lanczos-tridiagonalize the "
        "Hessian in the Krylov space of the gradient (Hessian-vector products), "
        "solve the tridiagonal subproblem exactly; Hessian-free, handles the "
        "indefinite/hard case; UNCONSTRAINED; alias \"trust-krylov\".\n\n"
        "Options:\n"
    ...
    symtab_set_docstring("NMinimize",
        "NMinimize[f, x]\n"
        ...
        "\tglobal minimum of f subject to the constraints cons (a single "
        "constraint, an And of constraints, or additional list elements "
        "{f, c1, c2, ...} that are implicitly And-ed).\n\n"
        "Variables may be given as bare symbols, {x, lo, hi} search-interval "
        "specs, or indexed variables x[i]; a held generator such as "
        "Table[x[i], {i, 1, n}] or Array[x, n] for the variables (and "
        "Table[...] for the constraints) is expanded automatically.\n\n"
        "Constraints may be equalities (==), inequalities (<, <=, >, >=), "
        "chained inequalities, and their And combinations.  Disjunctive (Or) "
        "constraints c1 || c2 are also supported: a point is feasible if it "
        "satisfies at least one branch.  Scalar integer "
        "variables are declared with Element[x, Integers] in the variable list "
        "or the constraints.  An empty feasible set returns "
        "{Infinity, {x -> Indeterminate, ...}}.\n\n"
        "Methods (Method -> ...):\n"
        "\tAutomatic            uses DifferentialEvolution.\n"
        "\t\"DifferentialEvolution\" DE/rand/1/bin with Deb feasibility rules; "
        "the default global engine.\n"
        "\t\"NelderMead\"          downhill-simplex search with random restarts.\n"
        "\t\"RandomSearch\"        multiple random starts refined by a local "
        "solver.\n"
        "\t\"SimulatedAnnealing\"  Metropolis search with geometric cooling.\n"
        "\t\"SHGO\"                Simplicial Homology Global Optimization "
        "(Endres, Sandrock & Focke 2018; cf. scipy.optimize.shgo): samples the "
        "bounded box, builds a graph, and starts one local search from each "
        "\"minimizer pool\" vertex (a point better than all its graph "
        "neighbours), so every basin is reached with few local searches.\n"
        "\t\"DualAnnealing\"       Generalized Simulated Annealing (Tsallis & "
        "Stariolo 1996; cf. scipy.optimize.dual_annealing): a heavy-tailed "
        "visiting distribution and a generalized Metropolis acceptance rule with "
        "reannealing, plus a local search after each Markov chain.\n"
        "\t\"DIRECT\"              DIviding RECTangles (Jones, Perttunen & "
        "Stuckman 1993; locally-biased DIRECT-L of Gablonsky & Kelley 2001; cf. "
        "scipy.optimize.direct): a deterministic Lipschitzian search that "
        "normalizes the box to the unit hypercube and repeatedly subdivides the "
        "\"potentially optimal\" cells (those on the lower-right hull of the "
        "(size, value) trade-off), needing no derivatives or random seed.\n"
        "\t\"BasinHopping\"        Monte-Carlo minimization (Wales & Doye 1997; "
        "cf. scipy.optimize.basinhopping): each step randomly perturbs the current "
        "point, LOCALLY MINIMIZES it (the \"quench\"), and accepts the move by a "
        "Metropolis rule on the two locally-minimized energies, with an adaptive "
        "step size targeting a fixed acceptance rate; strong on funnel-shaped "
        "landscapes.\n"
        "\tThe global best is polished with the exact local optimizer.  A "
        "method may be given with sub-options as {\"Name\", \"SearchPoints\" -> "
        "n, \"ScalingFactor\" -> F, \"CrossProbability\" -> cr, \"RandomSeed\" "
        "-> s}.  \"NelderMead\" also takes the simplex coefficients "
        "\"ReflectRatio\" (default 1), \"ExpandRatio\" (default 2), "
        "\"ContractRatio\" (default 0.5), \"ShrinkRatio\" (default 0.5), and "
        "\"Tolerance\" (convergence threshold), and \"InitialPoints\" -> "
        "{{x1,...}, ...} to seed the initial simplex.  \"SimulatedAnnealing\" "
        "takes \"SearchPoints\" -> K (number of annealing chains, default 1), "
        "\"PerturbationScale\" -> s (trial-step scale, default 1), and "
        "\"BoltzmannExponent\" -> f (uphill acceptance probability Exp[f[i, df, "
        "f0]]; Automatic keeps the built-in -df/T).  \"SHGO\" takes "
        "\"SamplingMethod\" -> \"Simplicial\" | \"Sobol\" | \"Halton\" (default "
        "\"Simplicial\": the exact Kuhn-triangulation graph, best for low "
        "dimension and falling back to \"Sobol\" above 7 variables; \"Sobol\" and "
        "\"Halton\" are low-discrepancy point sets whose connectivity is a "
        "k-nearest-neighbour graph, a documented approximation of scipy's "
        "Delaunay pool), \"SearchPoints\" -> n (number of sampling points, "
        "default 100), \"Iterations\" -> k (sampling/refinement rounds, default "
        "1; stops early once no new local minimum appears), and \"RandomSeed\" "
        "-> s (the QMC digital/fractional shift).  \"DualAnnealing\" takes "
        "\"VisitingParameter\" -> qv (visiting-distribution shape in (1, 3], "
        "default 2.62), \"AcceptanceParameter\" -> qa (acceptance shape in "
        "[-1e4, -5], default -5), \"InitialTemperature\" -> T0 (default 5230), "
        "\"RestartTemperatureRatio\" -> r (reanneal once the temperature falls "
        "below T0 r, default 2*^-5), \"LocalSearch\" -> True | False (run the "
        "per-chain local search, default True), \"SearchPoints\" -> K "
        "(independent chains, default 1), and \"RandomSeed\" -> s; MaxIterations "
        "is the per-chain temperature-step budget (default 1000).  "
        "\"DIRECT\" takes \"LocallyBiased\" -> True | False (True, the default, "
        "is DIRECT-L, biased toward the incumbent and best for few minima; False "
        "is the original unbiased DIRECT, better for many minima), \"Epsilon\" -> "
        "e (potentially-optimal slack, default 1*^-4), \"MaxFunctionEvaluations\" "
        "-> m (objective-evaluation budget, default 1000 n), \"MaxIterations\" -> "
        "k (division-round budget, default 1000), \"VolumeTolerance\" -> v and "
        "\"LengthTolerance\" -> l (stop once the incumbent cell is smaller than "
        "these fractions of the box, defaults 1*^-16 and 1*^-6), and \"MinValue\" "
        "-> f* with \"MinValueTolerance\" -> rt (stop once within relative rt of "
        "a known optimum f*).  "
        "\"BasinHopping\" takes \"Temperature\" -> T (Metropolis temperature, "
        "default 1), \"StepSize\" -> s (initial random-displacement half-width, "
        "default 0.5), \"StepInterval\" -> k (hops between step-size adaptations, "
        "default 50), \"TargetAcceptanceRate\" -> r (the acceptance rate the "
        "adaptation aims for, default 0.5), \"StepFactor\" -> a (step-size "
        "adjustment factor in (0,1), default 0.9), \"SuccessIterations\" -> m "
        "(stop a run once the best stalls for m hops; Automatic disables it), "
        "\"SearchPoints\" -> K (independent multi-start runs, default 1), and "
        "\"RandomSeed\" -> seed; MaxIterations is the hop count (default 100).  "
        "\"PostProcess\" (any "
        "method) controls the final exact local polish: True | Automatic | a "
        "named local method (\"InteriorPoint\", \"FindMinimum\", \"KKT\", ...) "
        "turn it on; False | None return the raw global-search point.  "
        "\"PenaltyFunction\" (any method) is the function applied to each "
        "constraint's violation when scoring infeasible points during the "
        "search: Automatic | None keep the built-in squared penalty; a pure "
        "function or function symbol (#^2 &, (10 #) &, Sqrt, ...) replaces it "
        "(Automatic is #^2 &).\n\n"
        "Options:\n"
        "\tMethod               global-search selector (see above).\n"
        "\tWorkingPrecision     MachinePrecision, or a positive digit count "
        "(MPFR refinement of unconstrained/box continuous problems).\n"
        "\tMaxIterations        Automatic or a positive integer cap on "
        "generations; default 100.\n"
        "\tAccuracyGoal         Automatic | Infinity | digits.\n"
        "\tPrecisionGoal        Automatic | Infinity | digits.\n"
        "\tEvaluationMonitor    :> body run on every objective evaluation.\n"
        "\tStepMonitor          :> body (accepted).\n\n"
        "NMinimize is Protected but NOT HoldAll (matching Mathematica); its "
        "variables should be unbound symbols, which evaluate to themselves.  "
        "During the search their values are set and restored Block-style, so an "
        "unbound variable is not left modified.  The search is deterministic for "
        "a fixed RandomSeed.  At "
        "MachinePrecision the objective and constraints are auto-compiled to "
        "bytecode for the trial-point loop, falling back to the interpreter "
        "where a construct cannot be compiled.  Expected numeric-domain messages "
        "(e.g. Power::infy from a 1/0 in a gradient on a non-differentiable "
        "ridge) are quieted during the search.  Returns {fmin, {x -> xmin, "
        "...}}.");
    symtab_set_docstring("NMaximize",
        "NMaximize[f, x]\n"
        ...
        "NMaximize (Protected, not HoldAll) shares NMinimize's methods, options, and "
        "constraint/domain handling.  Internally maximises by minimising -f, "
        "then negates the objective value in the result.  Returns "
        "{fmax, {x -> xmax, ...}}.");
    ...
    symtab_set_docstring("NLimit",
        ...
        "Method -> Automatic (default) runs both Richardson/Romberg and Wynn's "
        "epsilon and keeps the most self-consistent estimate, so branch-point / "
        "fractional-power approaches (which Richardson alone mishandles) are "
        "resolved accurately (Levin's u-transform also participates when the "
        "samples are settling). Method -> EulerSum forces Richardson/Romberg; "
        "Method -> SequenceLimit forces Wynn's epsilon; Method -> \"Levin\" "
        "forces Levin's transformation (\"LevinU\" | \"LevinT\" | \"LevinV\" "
        "select the u/t/v variant). expr must be numerical when z is numerical. "
        "Small spurious residuals are not recognised as zero -- Chop if needed. "
        "An expression whose sampled values oscillate with a non-decaying "
    ...
    symtab_set_docstring("NSum",
        ...
        "(HoldAll). Method -> Automatic picks Euler-Maclaurin for a monotone "
        "series whose summand extends off the integers, the Cohen-Villegas-Zagier "
        "method for alternating series, and Wynn's epsilon (partial-sum "
        "acceleration) otherwise, with Levin's u-transform as a last resort; an "
        "integer-only summand such as 1/Prime[n] cannot use Euler-Maclaurin (it "
        "has no continuous tail integral) and is extrapolated instead. Large "
        "finite sums use the difference of two infinite tails. Method -> \"Levin\" forces Levin's transformation "
        "(\"LevinU\" | \"LevinT\" | \"LevinV\" select the u/t/v variant). Machine "
        "or arbitrary precision via WorkingPrecision.\n\n"
        "Options: Method (Automatic | EulerMaclaurin | AlternatingSigns | "
        "WynnEpsilon | \"Levin\"), WorkingPrecision (default MachinePrecision), NSumTerms "
    ...
    symtab_set_docstring("Eigenvalues",
        ...
        "\n"
        "Options:\n"
        "    Cubics    -> False      (radicals for a general cubic; True forces them)\n"
        "    Quartics  -> False      (radicals for a general quartic; True forces them)\n"
        "    Method    -> Automatic  (numeric-matrix method dispatch)\n"
        "  With Cubics/Quartics -> False (the default) a general irreducible cubic\n"
        "  or quartic characteristic polynomial is returned as held Root[] objects.\n"
        "  Special always-solvable families -- binomials, quadratic-in-x^m, and\n"
        "  biquadratic-after-depression quartics -- are always returned in radical\n"
        "  form regardless of these options.\n"
        "\n"
    ...
    symtab_set_docstring("JordanDecomposition",
        "JordanDecomposition[m]\n"
        "\tgives the Jordan decomposition of a square matrix m as a list\n"
        "\t{s, j}, where j is the Jordan canonical form of m and s is the\n"
        "\tsimilarity matrix, so that m == s . j . Inverse[s].  j is\n"
        "\tblock-diagonal in Jordan blocks (an eigenvalue on the diagonal\n"
        "\tand 1's on the superdiagonal); the columns of s are the\n"
        "\tgeneralized eigenvectors grouped into Jordan chains that match\n"
        "\tj's blocks.  A 1 on j's superdiagonal marks a deficient\n"
        "\teigenspace.\n"
        "\n"
        "JordanDecomposition works on every input family supported by the\n"
        "rest of the linear-algebra builtins:\n"
        "\t- exact integer / rational matrices (output stays exact)\n"
        "\t- free-symbolic matrices with closed-form eigenvalues\n"
        "\t- complex matrices\n"
        "\t- machine-precision Real matrices (numeric eigensolver; the\n"
        "\t  generic distinct-spectrum case is diagonalizable, so j is\n"
        "\t  diagonal)\n"
        "\t- arbitrary-precision MPFR matrices (output at the input\n"
        "\t  precision)\n"
        "\n"
        "For exact / symbolic input the eigenvalues come from the\n"
        "characteristic polynomial and the block structure from the\n"
        "nullity sequence of (m - lambda I)^k.  For inexact input a\n"
        "diagonalizable matrix takes the numeric eigenvector path; a\n"
        "numerically defective matrix is rationalised, decomposed exactly,\n"
        "and numericalised back.  An exact matrix with an irrational,\n"
        "defective eigenvalue whose generalized eigenspace cannot be\n"
        "spanned exactly is left unevaluated rather than returned wrong.\n"
        "\n"
        "A non-square or empty matrix emits JordanDecomposition::matsq and\n"
        "the call is left unevaluated.");
    symtab_set_docstring("SchurDecomposition",
        "SchurDecomposition[m]\n"
        "\tgives the Schur decomposition of a numerical square matrix m as\n"
        "\ta list {q, t}, where q is orthonormal (unitary) and t is block\n"
        "\tupper-triangular (the Schur form), so that\n"
        "\tm == q . t . ConjugateTranspose[q].\n"
        "SchurDecomposition[{m, a}]\n"
        "\tgives the generalized (QZ) Schur decomposition as {q, s, p, t}\n"
        "\twith q, p orthonormal and s, t upper-triangular, so that\n"
        "\tm == q . s . ConjugateTranspose[p] and\n"
        "\ta == q . t . ConjugateTranspose[p].\n"
        "\n"
        "Options:\n"
        "\tPivoting -> True also returns a scaling/permutation matrix d\n"
        "\t  with m . d == d . q . t . ConjugateTranspose[q].\n"
        "\tRealBlockDiagonalForm -> True (default) keeps t real with 2x2\n"
        "\t  blocks for complex-conjugate eigenvalue pairs; -> False makes\n"
        "\t  t complex upper-triangular.\n"
        "\tTargetStructure -> \"Dense\" (default) | \"Structured\"; both\n"
        "\t  return dense matrices.\n"
        "\n"
        "SchurDecomposition is numerical: machine-precision Real/complex\n"
        "matrices (and NDArray / packed arrays) use LAPACK, and an\n"
        "arbitrary-precision real matrix uses the in-house MPFR Hessenberg +\n"
        "Francis QR at the input precision.  Arbitrary precision is native\n"
        "only for the standard real form with the default options; the\n"
        "complex, generalized, RealBlockDiagonalForm -> False, and Pivoting\n"
        "cases are computed at machine precision even for a high-precision\n"
        "input.  A non-numeric (symbolic) matrix, for which there is no\n"
        "closed-form Schur decomposition, is left unevaluated, as is a\n"
        "non-square argument.");
    ...
    symtab_set_docstring("DSolve",
        ...
        "Every returned branch is verified by back-substitution before it is kept.\n"
        "\n"
        "DSolve is a cascade polyalgorithm: DSolve[eqn, y, x, Method -> \"<Name>\"]\n"
        "dispatches directly to one method (strict, no fallback), and every method is\n"
        "also callable on its own as DSolve`<Name>[...] with its own ?-documentation.\n"
        "Methods (tried roughly in this order):\n"
        "\n"
        "First order:\n"
        "  Quadrature              y^(n)[x] == f(x): integrate n times.\n"
        "  LinearFirstOrder        y'[x] + p y == q: integrating factor.\n"
        "  Separable               y'[x] == g(x)h(y).\n"
        "  Bernoulli               y'[x] == A y[x] + B y^n[x].\n"
        "  Homogeneous             y'[x] == F(y[x]/x).\n"
        "  Exact                   M + N y'[x] == 0, M_y == N_x (+ mu(x)/mu(y) factor search).\n"
        "  Clairaut                y[x] == x y'[x] + f(y'[x]) (+ singular envelope).\n"
        "  Riccati                 y'[x] == q0 + q1 y[x] + q2 y[x]^2: linearize to 2nd-order.\n"
        "  Lagrange                y == x phi(y'[x]) + psi(y'[x]) (parametric solution).\n"
        "  Chini                   y'[x] == f y[x]^n + g y[x] + h (reducible-to-autonomous).\n"
        "  Abel                    y'[x] == f3 y[x]^3 + f2 y[x]^2 + f1 y[x] + f0.\n"
        "  FirstOrderSubstitution  y'[x] == F(a x + b y[x] + c).\n"
        "  Factorable              factor in y'[x], solve each factor.\n"
        "  NthAlgebraic            algebraic in the top derivative.\n"
        "  AlmostLinear            f(x)g(y[x])y'[x] + k(x)l(y[x]) + m(x) == 0.\n"
        "  LinearCoefficients      y'[x] == (a1 x + b1 y[x] + c1)/(a2 x + b2 y[x] + c2).\n"
        "  SeparableReduced        x y'[x]/y == G(x^n y[x]).\n"
        "  LieSymmetry (LieGroup)  heuristic Lie point-symmetry backstop.\n"
        "Linear constant-coefficient:\n"
        "  LinearConstantCoefficients   any order, homogeneous + inhomogeneous.\n"
        "  UndeterminedCoefficients     tidy particular for poly/exp/sinusoid forcing.\n"
        "Linear variable-coefficient / 2nd order:\n"
        "  EulerCauchy             a_n x^n y[x]^(n) + ... == g.\n"
        "  ExactODE                higher-order exact linear equation.\n"
        "  NormalForm              y''[x] + P y'[x] + Q y[x] -> z'' == r z (Kovacic prerequisite).\n"
        "  SpecialFunctionForm     Airy/Bessel/Kummer/Gauss recognizers.\n"
        "  Kovacic                 Liouvillian solutions of z''==r z.\n"
        "  FrobeniusSeries         series about a regular singular point.\n"
        "  PowerSeries             series about an ordinary point.\n"
        "  FirstOrderPowerSeries   order-1 power series (pinned-only).\n"
        "  OperatorFactor (DFactor)  factor a linear operator into first-order factors.\n"
        "Nonlinear higher-order:\n"
        "  ReductionOfOrder        y''[x] == F(x,y'[x]) (missing y).\n"
        "  AutonomousReduction     y''[x] == f(y[x],y'[x]) (missing x).\n"
        "  Liouville               y''[x] + g(y[x]) y'[x]^2 + h(x) y'[x] == 0.\n"
        "Systems (nfun>1):\n"
        "  DecoupleSystem          each equation involves one function.\n"
        "  TriangularSystem        dependency graph is a DAG.\n"
        "  LinearFirstOrderSystem  Y'==A Y+b(x), any constant A (Jordan e^{Ax}).\n"
        "First-order PDE:\n"
        "  PDELinearFirstOrder     a u_{v1}+b u_{v2}+c u==f (characteristics).\n"
        "\n"
        "Options: GeneratedParameters (constant head, default C), Assumptions, Method,\n"
        "IncludeSingularSolutions.  Attributes: Protected.");
    ...
    symtab_set_docstring("Order", "Order[e1, e2] gives 1 if e1 is before e2 in canonical order, -1 if e1 is after e2, and 0 if e1 is identical to e2.\nOrder compares structurally (the same canonical order as Sort), not by numerical value, and is compilable.");
    ...
    symtab_set_docstring("AlgebraicNumber",
        "AlgebraicNumber[theta, {c0, c1, ..., cn}]\n"
        "\trepresents the algebraic number c0 + c1 theta + ... + cn theta^n in the\n"
        "\tfield Q(theta).\n"
        "The generator theta may be given as a radical, a Root object, or another\n"
        "\tAlgebraicNumber; the coefficients ci must be integers or rationals.\n"
        "The object is automatically reduced so that theta is an algebraic integer\n"
        "\tand the coefficient list has length equal to the degree of the minimal\n"
        "\tpolynomial of theta.  AlgebraicNumber objects in the same field are\n"
        "\tcombined by arithmetic; those representing a rational number reduce to\n"
        "\texplicit rational form.  They are treated as numeric quantities: N gives\n"
        "\ttheir value to any precision and RootReduce converts them to Root\n"
        "\tobjects.  Attributes: NHoldAll, Protected.");
    symtab_set_docstring("ToNumberField",
        "ToNumberField[a, theta]\n"
        "\texpresses the algebraic number a in the field Q(theta) as an\n"
        "\tAlgebraicNumber object, or remains unevaluated if a is not in Q(theta).\n"
        "ToNumberField[{a1, a2, ...}, theta] expresses each ai in Q(theta).\n"
        "ToNumberField[{a1, a2, ...}] expresses the ai in a common extension field\n"
        "\tgenerated by a single algebraic number; ToNumberField[{a1, ...}, All]\n"
        "\tuses the smallest common extension field.\n"
    ...
    symtab_set_docstring("AlgebraicNumberDenominator",
        "AlgebraicNumberDenominator[a]\n"
        "\tgives the smallest positive integer n such that n a is an algebraic\n"
        "\tinteger.  a may be a rational, a radical, a Root object, or an\n"
        "\tAlgebraicNumber object; for an algebraic integer the denominator is 1.\n"
        "\tThreads over lists.  Requires FLINT.  Attributes: Listable, Protected.");
    symtab_set_docstring("AlgebraicNumberNorm",
        "AlgebraicNumberNorm[a]\n"
        "\tgives the field norm of the algebraic number a: the product of the\n"
        "\tconjugates of a over the rationals, equivalently the product of the\n"
        "\troots of a's minimal polynomial.  a may be an integer, a rational, a\n"
        "\tradical, GoldenRatio, a Root object, or an AlgebraicNumber object.\n"
        "\tAlgebraicNumberNorm[a, Extension -> theta] gives the norm relative to\n"
        "\tthe field Q(theta), for a an element of Q(theta).  Threads over lists.\n"
        "\tRequires FLINT.  Attributes: Listable, Protected.");
    symtab_set_docstring("AlgebraicNumberTrace",
        "AlgebraicNumberTrace[a]\n"
        "\tgives the field trace of the algebraic number a: the sum of the\n"
        "\tconjugates of a over the rationals, equivalently the sum of the\n"
        "\troots of a's minimal polynomial.  a may be an integer, a rational, a\n"
        "\tradical, GoldenRatio, a Root object, or an AlgebraicNumber object.\n"
        "\tAlgebraicNumberTrace[a, Extension -> theta] gives the trace relative to\n"
        "\tthe field Q(theta), for a an element of Q(theta).  Threads over lists.\n"
        "\tRequires FLINT.  Attributes: Listable, Protected.");
    symtab_set_docstring("NumberFieldIntegralBasis",
        "NumberFieldIntegralBasis[a]\n"
        "\tgives an integral basis for the field Q(a) generated by the algebraic\n"
        "\tnumber a: a list of algebraic integers that generate the ring of\n"
        "\tintegers O_K of Q(a) as a module over the integers.  The equation order\n"
        "\tZ[a] is enlarged to O_K by Round 2 (Pohst-Zassenhaus) when it is not\n"
        "\talready maximal, so the basis is correct for non-monogenic fields.  a\n"
        "\tmay be a radical, a Root object, or an AlgebraicNumber object; the basis\n"
        "\tit produces may not be unique.  Threads over lists.  Requires FLINT.\n"
        "\tAttributes: Listable, Protected.");
    symtab_set_docstring("AlgebraicIntegerQ",
        "AlgebraicIntegerQ[x]\n"
        "\tgives True if x is an algebraic integer (a root of a monic polynomial\n"
        "\twith integer coefficients) and False otherwise.  Rational integers are\n"
        "\talgebraic integers; non-integer rationals and non-algebraic quantities\n"
        "\tare not.  Requires FLINT.  Attribute: Protected.");
