...
Throughout this section, $R(t)\in\C[t]$ denotes a polynomial of degree $3$ or $4$ with simple roots. We write $C := \{(t,y)\in\overline{\C^2} : y^2 = R(t)\}$ for the smooth projective completion of the affine curve $y^2=R(t)$; this is a curve of genus~$1$. We are interested in the integrals
...
\begin{remark}\label{rmk:degree-2}
For $R$ of degree $2$, one can homogenize and view $R$ as a cubic with one root at infinity. The same theorem applies, with the understanding that the order-$3$ M\"obius transformation $S$ now permutes two finite roots and the point at infinity cyclically. A representative example is given in Section~\ref{sec:examples}, Example~\ref{ex:cubic-degree-2}.
\end{remark}
...
\begin{example}
Consider $R(t)=t^2-1$, viewed as a cubic with one root at infinity (cf.\ Remark~\ref{rmk:degree-2}). The three ramification points of the cube-radical curve $y^3 = R(t)$ are $\{1,-1,\infty\}$.

The order-$3$ M\"obius transformation cyclically permuting them as $1 \mapsto -1 \mapsto \infty \mapsto 1$ is found by solving the three linear conditions
\[
  S(1) = -1,\qquad S(-1) = \infty,\qquad S(\infty) = 1.
\]
Writing $S(t) = (At+B)/(Ct+D)$, the second condition forces $C = D$, the third forces $A = C$, and the first then gives $B = -3A$. Setting $A=C=D=1$:
\[
  S(t) \;=\; \frac{t-3}{t+1}.
\]
A direct check confirms $S(1) = -2/2 = -1$, $S(-1) = -4/0 = \infty$, $S(\infty) = 1$. The fixed points solve $(t-3)/(t+1) = t$, i.e.\ $t^2 + 3 = 0$, giving
\[
  \alpha = i\sqrt{3},\qquad \beta = -i\sqrt{3}.
\]
...
\]
so in the notation of Lemma~\ref{lem:cyclic-canonical} we have $c=4$, $K=1$. The function~\eqref{eq:H-formula} for $F=1$ is
\[
  H(z) \;=\; \frac{(\alpha-\beta)}{c^{1/3}(1-z)} \;=\; \frac{2i\sqrt{3}}{\sqrt[3]{4}\,(1-z)} \;=\; \frac{c_0}{1-z},\qquad c_0 := \frac{2i\sqrt{3}}{\sqrt[3]{4}}.
\]
Crucially, $H$ is \emph{not} a function of $z^3$ alone: the simple pole at $z=1$ produces a non-trivial $V_1$ component. The standard identity
\[
  \frac{1}{1-z} \;=\; \frac{1+z+z^2}{1-z^3}
\]
gives the eigendecomposition immediately:
\[
  H_0 \;=\; \frac{c_0}{1-z^3},\qquad H_1 \;=\; \frac{c_0\,z}{1-z^3},\qquad H_2 \;=\; \frac{c_0\,z^2}{1-z^3}.
\]
Since $H_1 \not\equiv 0$, Theorem~\ref{thm:cubic-goursat} concludes:
\begin{equation*}
  \boxed{\;\int \frac{\d t}{\sqrt[3]{t^2-1}} \quad\text{is \emph{not} elementary.}\;}
\end{equation*}

This contrasts sharply with the elementary square-root analog $\int \d t/\sqrt{t^2-1} = \log(t+\sqrt{t^2-1})$ (or equivalently $\operatorname{arccosh} t$). The difference is geometric: the curve $y^2 = t^2-1$ has genus $0$ (it is a smooth conic), but the curve $y^3 = t^2-1$ has genus $1$. Goursat's framework can detect pseudo-elementarity, but for $\int \d t/\sqrt[3]{t^2-1}$ it correctly identifies the obstruction.
...
\]
a non-elementary Abelian integral on the cube-root elliptic curve $y^3=x(x-1)$. The integrals $J_0$ and $J_2$ contribute elementary pieces (with $\arctan$ and $\log$ terms via the parametrization~\eqref{eq:Y0-param} and the trivial reduction for $J_2$), but they cannot fully cancel the transcendental $J_1$.

Numerically, this is consistent with the fact that symbolic-integration systems return $\int \d t/\sqrt[3]{t^2-1} = t\,\mathrm{e}^{-i\pi/3}\,{}_2F_1(\tfrac{1}{3},\tfrac{1}{2};\tfrac{3}{2}; t^2)$, a hypergeometric expression with no closed form in elementary functions. \hfill$\square$
\end{example}
...
Theorem~\ref{thm:cubic-goursat} extends naturally to $n$-th roots, considering integrands of the form $F(t)\,\d t/\sqrt[n]{R(t)}$ and $\Z/n$-symmetries of $R$. Under an order-$n$ M\"obius transformation $S$, the function $H$ associated to the integrand decomposes into $n$ eigencomponents under $z\mapsto \zeta z$ ($\zeta = e^{2\pi i/n}$). After the substitution $x=z^n$, the $k$-th eigencomponent leads to the curve $y^n = x^{n-k}(x-K)$. The genus of this curve depends on $\gcd(k,n)$:
\[
  g\bigl(y^n = x^{n-k}(x-K)\bigr) \;=\; \begin{cases} 0 & \text{if } \gcd(k,n) > 1 \text{ or } k\in\{0,n-1\}, \\ \text{positive} & \text{otherwise.} \end{cases}
\]
For $n=2$ we recover Goursat's theorem (only one non-trivial eigencomponent, always elementary). For $n=3$ we recover Theorem~\ref{thm:cubic-goursat} (two elementary, one elliptic). For $n=5$ there are four non-trivial eigencomponents, two elementary and two genuinely transcendental, and the obstruction corresponds to differentials on a curve of higher genus.
