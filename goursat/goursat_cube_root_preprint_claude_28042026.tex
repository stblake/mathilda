...
\end{lstlisting}
returns $\tfrac{1}{2}(t^3-1)^{2/3}$, the closed form of Example~\ref{ex:Ft2} obtained by integrating $\text{J2integrand}(u) = u$ and back-substituting $u\mapsto (t^3-1)^{1/3}$. The same form correctly handles non-elementary inputs: \texttt{IntegrateGoursat[t/(t\textasciicircum 3 - 1)\textasciicircum(1/3), t]} returns the diagnostic Association reporting non-elementarity rather than attempting a closed form.
