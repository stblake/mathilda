...
Let $R(t) \in \C[t]$ be a quartic polynomial whose four roots are distinct and in harmonic position, $S$ the corresponding order-$4$ cyclic Möbius transformation, $\alpha, \beta$ its fixed points, $c, K$ the constants of Lemma~\ref{lem:cf-n4}, and $H_{1/4}$ the function defined in~\eqref{eq:Hp-def}. Decompose $H_{1/4} = H_{1/4,0} + H_{1/4,1} + H_{1/4,2} + H_{1/4,3}$ via~\eqref{eq:projection-Vk}, and write $H_{1/4,k}(z) = z^k\varphi_{1/4,k}(z^4)$ for unique rational functions $\varphi_{1/4,k}\in\C(x)$.
\begin{enumerate}[(i)]
  \item \textbf{(Sufficient criterion for elementarity.)} If $H_{1/4,1} \equiv 0$ and $H_{1/4,2} \equiv 0$, then $\int F(t)\,\d t / R(t)^{1/4}$ is elementary, with antiderivative the sum of two pieces:
  \begin{itemize}
    \item $J_0 = \int \varphi_{1/4,0}(K/(1-w^4))\,w^2\,\d w/(1-w^4)$, a rational integral in $w$ via Lemma~\ref{lem:Y0-param} with $w = y/x$ in canonical coordinates (i.e., $w$ defined so that $x = K/(1-w^4)$);
    \item $J_3 = \int \varphi_{1/4,3}(u^4 + K)\,u^2\,\d u$, a rational integral in $u$ via Lemma~\ref{lem:Y3-param} with $u = (x-K)^{1/4}$.
  \end{itemize}
  \item \textbf{(Sufficient criterion for non-elementarity.)} If $H_{1/4,1} \not\equiv 0$ or $H_{1/4,2} \not\equiv 0$, and the corresponding descended differentials $\omega_{1/4,k} = \varphi_{1/4,k}(x)\,\d x/[4\,(x^{3-k}(x-K))^{1/4}]$ on $Y_k^{(1/4)}$ are of the second kind (zero residue at every pole), then $\int F\,\d t/R^{1/4}$ is not elementary.
  \item \textbf{(A common sufficient condition for (ii).)} If $\varphi_{1/4,1}$ and $\varphi_{1/4,2}$ are polynomials --- which holds in particular when $F\in\C[t]$ is a polynomial --- then the corresponding obstruction differentials are automatically of the second kind, so the criterion in (ii) reduces to the test $H_{1/4,1}\not\equiv 0$ or $H_{1/4,2}\not\equiv 0$.
\end{enumerate}
\end{theorem}
...
\]
where, after the substitution $x = z^4$, each $J_k$ is an Abelian integral on $Y_k^{(1/4)}$. By Lemma~\ref{lem:genera}, $J_0$ and $J_3$ live on genus-$0$ curves, parametrized rationally by Lemmas~\ref{lem:Y0-param} and~\ref{lem:Y3-param}, hence elementary. By Lemmas~\ref{lem:Y1-lem} and~\ref{lem:Y2-aut}, $J_1$ and $J_2$ live on the elliptic curve $E\colon y^2 = u^3 + Ku$. The reductions in (i) follow by direct substitution as in~\cite{GoursatCubic}.

For (ii), recall the structure of meromorphic differentials on a curve $C$ of positive genus: a non-zero \emph{second-kind} differential (one with zero residue at every pole) is never elementary, because its non-vanishing class in $H^1_{\mathrm{dR}}(C)/\d\C(C)$ obstructs an exact representation~\cite[Ch.\ 5]{Bronstein2005}. If both $\omega_{1/4,1}$ and $\omega_{1/4,2}$ are of the second kind and at least one is non-zero, the corresponding $J_k$ is non-elementary; since the eigenspaces $V_1$ and $V_2$ live on different elliptic-curve images of $E$ that are linearly independent in the relevant cohomology, the sum cannot cancel. Hence the integral is not elementary.

For (iii), we verify the second-kind condition for polynomial $\varphi_{1/4,k}$. The branch points of $Y_k^{(1/4)} \to \mathbb{P}^1$ via $x$ are at $x=0, K, \infty$. At each branch point, a local uniformizer $\tau$ on $Y_k^{(1/4)}$ provides a local description of $\d x/y$ as a Laurent series in $\tau$ whose coefficient of $\tau^{-1}$ vanishes (the local exponents preclude a residue, by the same argument as in Theorem 4.3(iii) of~\cite{GoursatCubic}). For polynomial $\varphi_{1/4,k}(x) = \sum c_d x^d$, the contribution at infinity is a Laurent series in $\tau$ with leading order strictly less than $-1$ (the orders never align with $\tau^{-1}$ for non-negative integer $d$), so the residue at infinity is also zero. Hence the differential is of the second kind. The polynomial-$F$ case follows because the canonical-form prefactor $(\alpha-\beta)(1-z)^{-1}$ contributes poles only at $z=1$ (or no factor when $\beta=\infty$), which under $x = z^4$ project to non-branch points only when $\beta$ is finite, and the residues there must again vanish for the differential to remain of the second kind --- a calculation we verify case-by-case for the explicit polynomial-$F$ examples in Section~\ref{sec:examples}.
\end{proof}
...
Let $R(t) \in \C[t]$ be a quartic polynomial in harmonic configuration as in Theorem~\ref{thm:main-14}, $H_{3/4}$ the function defined in~\eqref{eq:Hp-def}, decomposed as $H_{3/4} = \sum_k H_{3/4,k}$, and write $H_{3/4,k}(z) = z^k\varphi_{3/4,k}(z^4)$.
\begin{enumerate}[(i)]
  \item \textbf{(Sufficient criterion for elementarity.)} If $H_{3/4, 0} \equiv 0$ and $H_{3/4, 1} \equiv 0$, then $\int F(t)\,\d t / R(t)^{3/4}$ is elementary, with antiderivative
  \begin{align*}
  ...
  \item \textbf{(Sufficient criterion for non-elementarity.)} If $H_{3/4, 0} \not\equiv 0$ or $H_{3/4, 1} \not\equiv 0$, and the corresponding descended differentials on $Y_k^{(3/4)}$ are of the second kind, then the integral is not elementary. The descended differentials live on the lemniscate $E$ via Corollary~\ref{cor:lemniscate-34}.
  \item \textbf{(A common sufficient condition for (ii).)} If $\varphi_{3/4,0}$ and $\varphi_{3/4,1}$ are polynomials --- which holds when $F\in\C[t]$ is a polynomial --- then the descended differentials are automatically of the second kind, and (ii) reduces to the test $H_{3/4,0}\not\equiv 0$ or $H_{3/4,1}\not\equiv 0$.
\end{enumerate}
\end{theorem}
...
\]
The non-elementarity argument in (ii) and the polynomial-$F$ verification in (iii) follow by the same arguments as in Theorem~\ref{thm:main-14}, with the eigenspace shift $V_2 \leftrightarrow V_0$ in the obstruction set.
\end{proof}
...
\]
For $p \in \{1/4, 3/4\}$, the package first tests the harmonic condition on the roots of $R$, returning \texttt{"non-harmonic"} if it fails (in which case the present theory does not apply); otherwise it computes the order-$4$ Möbius transformation $S$, its fixed points, the canonical-form constants $c, K$, and the four eigenspace projections of $H_p$ from~\eqref{eq:Hp-def}, returning \texttt{"status" $\to$ "elementary"} or \texttt{"obstructed"} according to the criterion of the appropriate theorem.
...
\end{lstlisting}
both return \texttt{"status"\,$\to$\,"obstructed"}: the first with obstruction $H_1 = z$, the second with $H_2 = z^2$. Both witness Abelian integrals on the lemniscate $y^2 = u^3 + u$.

\paragraph{Example~\ref{ex:F1-34} (p = 3/4, the duality flip).}
\begin{lstlisting}[basicstyle=\footnotesize\ttfamily,frame=none]
In[]:= IntegrateGoursat[1, t^4 - 1, t, 3/4]
\end{lstlisting}
returns \texttt{"status"\,$\to$\,"obstructed"} with obstruction $H_0 = 1$. The same input that was elementary at exponent $1/4$ is non-elementary at $3/4$, as predicted by the $V_0 \leftrightarrow V_2$ duality.
