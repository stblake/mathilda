...
%%% Mathematica listing style
\definecolor{mmaKey}{rgb}{0.0, 0.2, 0.6}
\definecolor{mmaCom}{rgb}{0.0, 0.45, 0.0}
\definecolor{mmaStr}{rgb}{0.6, 0.0, 0.0}
\lstdefinelanguage{Mathematica}{
  morekeywords={Module,If,Solve,DeleteDuplicates,Length,Return,Together,
    Simplify,Integrate,PossibleZeroQ,Coefficient,Cancel,Expand,FirstPosition,
    RotateLeft,First,Sum,Switch,MemberQ,Sqrt,Exp,Pi,True,False,With,Map,
    AppendTo,FreeQ,Block,Function,Table,List,Equal,Set,SetDelayed,Rule,
    RuleDelayed,Replace,ReplaceAll,Print,Association,Infinity},
  morecomment=[s]{(*}{*)},
  morestring=[b]",
  sensitive=true,
}
\lstset{
  ...
  keywordstyle=\color{mmaKey},
  commentstyle=\color{mmaCom}\itshape,
  stringstyle=\color{mmaStr},
...
\setcounter{tocdepth}{1} % top-level sections only in the TOC
...
We provide a self-contained {\sc Mathematica} package implementing the diagnostic algorithms of Theorems~\ref{thm:goursat2} and~\ref{thm:cubic-goursat}. The entry point is
\[
  \texttt{PseudoEllipticIntegrate[F, R, t, n]},\qquad n\in\{2,3\},
\]
which takes a rational integrand $F\in\C(t)$ and a polynomial $R\in\C[t]$ with simple roots, computes the relevant character or eigenspace projections, and returns a Mathematica association keyed by \texttt{"status"}. Two outcomes are possible:
\begin{itemize}
  \item \texttt{"status"\,$\to$\,"elementary"}: the projection criterion of the relevant theorem is satisfied. The returned association contains the explicit reduction to a rational integrand --- via Theorem~\ref{thm:goursat1} for $n=2$ (yielding $\int G(x)\,\d x/\sqrt{Q(x)}$ with $Q$ quadratic) and via the parametrizations of $Y_0$ and $Y_2$ for $n=3$ (yielding rational integrands in the parameter variables $w$ and $u$, with no radicals remaining).
  \item \texttt{"status"\,$\to$\,"non-elementary"}: the criterion fails, and the obstruction is returned --- the $V_4$-trivial component $F^{(0)}$ for $n=2$, or the $\omega$-eigencomponent $H_1$ for $n=3$.
\end{itemize}

Crucially, the implementation never invokes Mathematica's \texttt{Integrate}: every step is the constructive procedure from the proofs of Theorems~\ref{thm:goursat1}--\ref{thm:cubic-goursat}, performed symbolically in rational arithmetic. The diagnostic and the reduction are therefore independent of Mathematica's symbolic-integration engine.
...
The cube-root branch (\texttt{CubicTest}) computes the order-$3$ M\"obius transformation of Lemma~\ref{lem:cyclic-mobius}, performs the change of variable $z = (t-\alpha)/(t-\beta)$ to obtain the function $H$ from~\eqref{eq:H-formula}, and decomposes $H$ via the projection formula~\eqref{eq:projection-Vk}. When the criterion is satisfied (i.e.\ $H_1\equiv 0$), the package extracts $\varphi_0(x)$ and $\psi_2(x)$ from the eigencomponents $H_0(z)=\varphi_0(z^3)$ and $H_2(z)=z^2\psi_2(z^3)$, then applies the parametrization $x = K/(1-w^3)$ for $Y_0$ and the inverse $u = \sqrt[3]{x-K}$ for $Y_2$ to produce \emph{purely rational} integrands in $w$ and $u$ for $J_0$ and $J_2$. The case $\beta=\infty$ (which arises whenever $0$ or $\infty$ is fixed by $S$, e.g.\ for $R(t)=t^3-1$ with $S(t)=\omega t$) is handled by the explicit closed form for the cyclic M\"obius transformation $S(t)=(at-(a^2-ab+b^2))/(t-b)$ derived in Remark~\ref{rmk:degree-2}.
...
PseudoEllipticIntegrate[F_, R_, t_, n_Integer:2] :=
  Switch[n, 2, GoursatTest[F, R, t],
            3, CubicTest[F, R, t],
            _, "Only n = 2 and n = 3 are supported"];
\end{lstlisting}
...
In[]:= PseudoEllipticIntegrate[t, (t^2 - 1)(t^2 - 4), t, 2]
\end{lstlisting}
...
\begin{lstlisting}[basicstyle=\footnotesize\ttfamily,frame=none]
In[]:= PseudoEllipticIntegrate[1, t^3 - 1, t, 3]
\end{lstlisting}
...
\begin{lstlisting}[basicstyle=\footnotesize\ttfamily,frame=none]
In[]:= PseudoEllipticIntegrate[t, t^3 - 1, t, 3]
\end{lstlisting}
...
\begin{lstlisting}[basicstyle=\footnotesize\ttfamily,frame=none]
In[]:= PseudoEllipticIntegrate[1, t^2 - 1, t, 3]
\end{lstlisting}
