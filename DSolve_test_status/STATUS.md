...
`ctest -R dsolve_corpus_2_1_2_tests` · gate baseline **619** (M59 Inactive + implicit first integrals).
...
| 2026-09-23 (**M59**) | **595 / 1204 total** | — | **609** | **+4 (591 → 595), 0 FAIL — deterministic +9.** `Inactive`/`Activate` primitive + the `AutonomousReduction` implicit companion: where the M58 explicit method declined a non-elementary stage-2 quadrature, it now returns the **inert first integral** `Inactive[Integrate][1/p, y[x]] − x == C[1]` (built + verified via the FTC/implicit-function rule WITHOUT running Integrate, so `y'==Sqrt[y Log y+…]`'s 45 s spin → 0.0 s). **The whole order-3 autonomous residue solves: 263/264/267/268/1167/1168 + dups 710/711/712** (`3rd_high_reducible` bucket 8 → 17 PASS). A numeric self-verify (reconstruct `y'..y⁽ⁿ⁾` from the reduction chain) is the correctness gate (0 FAIL). The 7 P→U (58/161 gained, 375/439/592/799/877/983/1153 lost) are all the x-dependent `_with_linear_symmetries` timing cluster — M59's autonomous companion never touches them; the 1 crash (1153) is the known intermittent fork SIGSEGV (clean in isolation). Gate baseline **628 → 619**. |
...
| 1st_with_symmetry     |  65 |  35 |  30 | 53.8 | **M57 SolvableForY +352** (`[F(x),G(y)]`-symmetry, solvable-for-y); residue = targeted Lie symmetry ansätze |
| 3rd_high_reducible    |  34 |  17 |  17 | 50.0 | **M59 implicit first integrals +9** (263/264/267/268/1167/1168 + dups → `Inactive[Integrate][1/p,y]−x==C[1]`; M58 +1 = 1143 explicit); residue = `_mu_y2`/exact-ODE 3rd-order families + missing-y downstream (236/1117/269) |
| 1st_solvable_for_yx   |  42 |  23 |  19 | 54.8 | **M57 SolvableForY/X** (the `y=G(x,y')` differentiation method; +347); residue = radical/`Root` branches, transcendental (gate-declined), and cases exceeding the 8 s budget (350/351) |
