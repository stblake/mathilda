...
TeXForm produces AMS-LaTeX-compatible TeX output. When an input evaluates to TeXForm\[expr\], TeXForm does not appear in the output. TeXForm translates standard mathematical functions and operations. Following standard mathematical conventions, single-character symbol names are given in italic font, while multiple character names are given in roman font.
