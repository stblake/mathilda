...
```mathematica
In[4]:= TrigToExp[Sin[x]]
Out[4]= (-1/2*I) E^(I x) + (1/2*I) E^(-I x)

In[5]:= TrigToExp[Cos[x]]
Out[5]= 1/2 E^(-I x) + 1/2 E^(I x)

In[6]:= TrigToExp[Cosh[x]]
Out[6]= 1/2 E^x + 1/2 E^(-x)

In[7]:= TrigToExp[ArcTan[x]]
Out[7]= (-1/2*I) Log[1 + I x] + (1/2*I) Log[1 - I x]

In[8]:= TrigToExp[ArcSin[x]]
Out[8]= -I Log[I x + Sqrt[1 - x^2]]
```
