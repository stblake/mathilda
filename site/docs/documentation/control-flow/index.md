...
- [`Check`](Check.md) — Check[expr, failexpr] returns failexpr if a message is generated while evaluating expr, otherwise the value of expr.  _(Stable)_
...
- [`Message`](Message.md) — Message[sym::tag, args] prints the named message unless messages are suppressed.  _(Stable)_
- [`Piecewise`](Piecewise.md) — Piecewise[{{val_1, cond_1}, {val_2, cond_2}, ...}]  _(Stable)_
- [`Quiet`](Quiet.md) — Quiet[expr] evaluates expr with messages suppressed and returns its value. Quiet[expr, spec] suppresses all messages regardless of spec.  _(Stable)_
