...
<style>
  ...
  .focused-view {
    ...
    /* The MENU BAR sits above the toolbar, so the strip to clear is the sum of both rows.
       These two numbers must agree with App.svelte's .app-bar.toolbar-mode height or the
       view either overlaps the bar or leaves a gap under it. */
    inset: calc(var(--menubar-h, 28px) + var(--toolbar-h, 46px)) 0 0 0;
