...
**Key characteristics:**
- ~365 kLoC of C99 across ~550 source modules (≈915 files with headers).
...
- [`COMPILE_MISSING.md`](COMPILE_MISSING.md) — the heads that have a numeric
  fast path in the interpreter and no `Compile[]` lowering (32 at last count),
  grouped by the mechanism that would close each group. Regenerated by
  `make check-compile-coverage`, which prints the current count.
- [`docs/experiments/`](docs/experiments/) — twenty execution-speed
  ...
  which enumerates every builtin instead of choosing kernels, and is what
  `tools/numeric_coverage.py` and `tools/numeric_sweep.py` re-run.
- [`benchmarks/`](benchmarks/) — the weekly gap-driven benchmark **job** (as
  opposed to `docs/experiments/`, which is the narrative record). 32 experiments
...
src/
  ...
  part.c / sort.c                             Part + sort (structural ops)
  ...
  compile/          Compile[] bytecode compiler: type inference
                    (compile_infer.c), per-category codegen (compile_emit_*.c),
                    VM, and auto-compilation (autocompile.c)
  list/             List builtins (Flatten, Accumulate, Differences, Join,
                    Gather, ...; one per file, hub in list_init.c)
  strings/          String functions (StringLength/Replace/Partition/...) and
                    the string-pattern engine
  sum/              Sum: finite and infinite series families (Gosper,
                    hypergeometric, rational, zeta, ...)
  product/          Product: finite and infinite product families
                    (telescoping, q-products, Euler, ...)
  numerical_calculus/  N-prefixed calculus (NIntegrate, NDSolve, NSum, ND, ...)
                    and numerical optimization (NMinimize, FindMinimum family)
  numerical_roots/  NSolve, NRoots, FindRoot
  graph/            Graph data structure, generators, and graph algorithms
  ml/               Machine-learning primitives (Classify, PCA, GMM, trees, ...)
  ffi/              Foreign-function interface / dynamic module loading
...
`core.c` registers its own builtins and then calls each subsystem's `*_init()`
function — several dozen of them, in a fixed order, spanning:

- **Foundation** — interned names and contexts (`sym_names`, `context`, `eval`).
- **Arithmetic & structural heads** — `parfrac`, `modular`, `list`, `complex`,
  `trig`, `hyperbolic`, `logexp`, `comparisons`, `boolean`.
- **Rules, patterns, control flow** — `replace`, `patterns`, `cond`, `iter`.
- **Special functions** — `gamma`, `zeta`, `bessel`, `polygamma`, `erf`, and the
  rest of `src/special_functions/`.
- **Packed/NDArray substrate** — `ndarray`, `ndkernels`, `ndinteger`, `pack`;
  plus `assoc`, `vectors`, and strings.
- **Numeric subsystems** — the `numerical_calculus`, `numerical_roots`, `sum`,
  and `product` modules.
- **Algebra & higher-level services** — `poly`, `facpoly`, `rat`, `expand`,
  `linalg`, `simp`, `stats`, `graph`, `ml`, `graphics`, `info`, `datetime`,
  `load`.

The ordered, authoritative list is the sequence of `*_init()` calls in
`core_init()` itself — treat `core.c` as the source of truth rather than this
summary.
...
source scan cannot match — many heads are defined in `src/internal/*.m`, not C),
...
Baselines (Apple M-series, `USE_ECM=OFF`, 2026-07-30) live in `bench_assoc.c` as
