...
The third sweep ([`HPC_SWEEP_2_APPLICATIONS.md`](../10-hpc-sweep-applications/README.md))
...
This sweep took the two worst application rows from 66.5× and 19× behind
NumPy to *ahead* of it, and put the running maximum ahead of
`np.maximum.accumulate`. What the closing probe still showed:

| row | Mathilda | best other | gap | cause |
|---|---:|---:|---:|---|
| 1-D `ListConvolve`, 5-tap over 10⁶ | 16.1 ms | 1.40 ms (NumPy) | **11.5×** | a per-output boundary test in the inner loop |
| General linear recurrence (EMA) | 22.6 ms | 3.02 ms (`lfilter`) | 7.5× | the compiled VM at 22 ns/element |
| `Tally`, 10⁷ | — | — | 7.2× | hashing, not boxing |
| `Part` with a span | 7.4 ms | — | — | still builds a position array per axis |
| `Reverse`, `RotateLeft` | — | — | 2.9–3.8× | serial |

### The road to fastest

1. **Split the convolution output range into boundary / interior / boundary.**
   The interior body needs no range test at all — the fourth sweep's odometer
   change removed the index division and left the test, which is now the whole
   cost. Expected: 16.1 ms → ~3 ms, close to NumPy.
2. **A `start/step/n` selector for spans** (plan 9.5), which makes
   `v[[a ;; b]]` a pure `memcpy` and also removes the position array from
   experiment 12's gather and experiment 5's strided sieve write. One change,
   three experiments.
3. **Thread the block moves** — `Reverse`, `RotateLeft` — see
   [`06-packed-arrays/`](../06-packed-arrays/README.md).
4. **The linear recurrence is genuinely sequential** and 22 ns/element is the
   VM's per-element dispatch. Closing this needs the codegen work in
   [`03-compile-optimising-codegen/`](../03-compile-optimising-codegen/README.md)'s
   roadmap rather than anything about scans.

Item 2 is the highest-leverage line in this list because three separate
experiments are waiting on it.
