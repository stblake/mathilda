...
> [`HPC_SWEEP_3_NUMPY_GAP.md`](../11-hpc-sweep-numpy-gap/README.md) §"A separate finding".
...
[`HPC_SWEEP_2_APPLICATIONS.md`](../10-hpc-sweep-applications/README.md).
...
## Where the slow rows went

This sweep's job was to find the gaps, not to close them; every row it
identified is carried by a later experiment, and the roadmap for each lives
there rather than being duplicated here:

| what this sweep found | now tracked in |
|---|---|
| `QRDecomposition`, `Eigenvalues`, `SVD` bypass LAPACK (6–14×) | [`07-blas-lapack-routing/`](../07-blas-lapack-routing/README.md) |
| The sieve at 25× NumPy, Game of Life at 2.8×, `fib` at 30× | [`05-machine-integers/`](../05-machine-integers/README.md) |
| `Reverse`/`RotateLeft` serial, `Sort` at 2× | [`06-packed-arrays/`](../06-packed-arrays/README.md) |
| `Sin`/`Exp` at 1.16× Mathematica | [`02-compile-array-fusion/`](../02-compile-array-fusion/README.md) |
| Structural ops and scans on the boxed path | [`11-hpc-sweep-numpy-gap/`](../11-hpc-sweep-numpy-gap/README.md) |
| Compositions slower than their parts | [`10-hpc-sweep-applications/`](../10-hpc-sweep-applications/README.md) |

The one thing this sweep should be read for on its own is the **method**: a
wide net over seven categories, run against two other systems, with a value
check on every row. Five of the six entries above were invisible until
something outside the tuned path was measured.
