...
the same lesson; see
[`HPC_SWEEP_3_NUMPY_GAP.md`](../11-hpc-sweep-numpy-gap/README.md).
...
After this sweep's seven fixes, two rows led Mathematica by 3–16× and two were
still far behind NumPy. Both of the latter were closed by the sweep that
followed, and the rest by the fifth:

| row | at the time | now | roadmap |
|---|---:|---|---|
| Return series (EMA, vol, drawdown) | 66.5× NumPy | closed — 2.78× *ahead* of Mathematica | [`11-hpc-sweep-numpy-gap/`](../11-hpc-sweep-numpy-gap/README.md) |
| Gaussian blur + Sobel | 19× NumPy | closed — 1.10× ahead of NumPy | [`11-hpc-sweep-numpy-gap/`](../11-hpc-sweep-numpy-gap/README.md) |
| k-means | 2.92× NumPy | unfused composition | plan 9.2 |
| Logistic regression | 3.06× NumPy | a copying `Transpose` | [`17-neural-network-training/`](../17-neural-network-training/README.md) |
| N-body, 3-D heat, Welch PSD | 1.4–2.3× NumPy | unfused composition | plan 9.2 |

**Everything remaining in this experiment is one item: interpreter-level
fusion.** `Compile[]` fuses (experiment 2) and ordinary array code does not,
so every one of these pipelines makes three to nine passes over its data where
a fused kernel would make one or two. That is the 2–3× band all five
outstanding rows sit in, and it is plan item 9.2.

The logistic-regression row additionally wants the transposed view of
experiment 17's roadmap, which is the same feature as experiment 14's sliding
window.
