...
[`MACHINE_INTEGERS.md`](../05-machine-integers/README.md), the gate had to materialise every
...
path with it. See [`HPC_SWEEP_2_APPLICATIONS.md`](../10-hpc-sweep-applications/README.md)
and [`HPC_SWEEP_3_NUMPY_GAP.md`](../11-hpc-sweep-numpy-gap/README.md).
...
Packing turned ten operations from 5.9× to 56000× faster *within Mathilda*.
Against the other two systems the picture afterwards is:

| row | Mathilda | best other | gap | cause |
|---|---:|---:|---:|---|
| `Sort`, 10⁷ | 319 ms | 156 ms (NumPy) | 2.05× | a comparison sort against NumPy's introsort |
| `Reverse`, 10⁷ | 33.1 ms | 11.3 ms (WL) | 2.93× | serial `memcpy` per row |
| `RotateLeft`, 10⁷ | 38.7 ms | 10.1 ms (WL) | 3.83× | serial, and two passes through a scratch buffer |
| `Total`, `Accumulate`, `Dot`, `Sin`, `Exp`, `triad`, `Differences` | — | — | 1.04–1.29× | at or near the memory floor |

The last group is within 30% of both systems and most of it is bandwidth; it
is not where the work is. Three rows are.

### The road to fastest

1. **Thread `Reverse` and `RotateLeft`.** Both are pure block moves —
   `nd_parallel_for` already exists and every elementwise kernel uses it, and
   neither has any ordering subtlety to worry about. Expected: 33–39 ms →
   ~12 ms, level with Mathematica.
2. **Rotate in one pass.** `nd_rotate_axes` copies through a scratch buffer,
   so a rank-1 rotate reads and writes 80 MB twice. For axis 0 it is two
   `memcpy`s into the destination directly, with no scratch at all.
3. **A better sort.** 319 ms for 10⁷ doubles is ~32 ns/element; NumPy's
   introsort does it in 15.6. A radix sort on the IEEE bit pattern is the
   standard answer for float64 keys and is typically 2–3× a comparison sort,
   which would put this row ahead of both. The int64 arm can share it.
4. **A vector math library for `Sin`/`Exp`** — see experiment 2's roadmap;
   worth ~1.9× on those two rows and applies system-wide.

Items 1–3 are all self-contained kernel work on paths that already exist.
