...
are left intact — they cite the external source, not picocas's symbol.
...
picocas's `Times` canonicaliser does not immediately recombine the
...
where Mathematica simplified an expression in ~100 ms but picocas
...
- Implementation uses `ITIMER_PROF` + `SIGPROF` so that the kernel
  ...
  ("CPU time spent inside the main PicoCAS kernel process"). The
