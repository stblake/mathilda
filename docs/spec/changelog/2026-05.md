...
| Call | Before | After Layer 0+5 | After Layer 0+1+3+5 |
|------|--------|-----------------|---------------------|
| `Together[diff, Extension -> Automatic]` | 190 ms | 2.5 ms | 2.5 ms |
| `Simplify[diff]` (user's case)            | 357 ms | 173 ms | 173 ms |
| `Simplify[D[Integrate[a x/(x^3+2), x], x]]` | ~190 ms | ~170 ms | **111 ms** |
