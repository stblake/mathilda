...
- `Together[Sqrt[A] - Sqrt[B], Extension -> Automatic]` where `A` and
  `B` are mathematically equal but structurally distinct still
  doesn't collapse: Phase F dedups the generator set but not the
  input expression, so the tower substitution loop misses one of the
  structurally-distinct radicals in the input.  Closing this requires
  running Phase F's canonicalisation on the input as well.

### Tests

`tests/test_simp_algebraic_cuberoot.c` adds 2 new tests
(`test_power_neg_pq_lift`, `test_multigen_qgamma_constant_collapse`).

### Regression risk

Full test suite 94 pass / 16 fail — identical pre-existing failure
