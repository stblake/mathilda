...
| `CoefficientDomain`  | `Rationals`      | `Rationals`, `Automatic`, `Polynomials[x, ...]` (named symbols become coefficient-ring parameters), `InexactNumbers[p]` (monic basis with `p`-digit real coefficients) |
...
- `CoefficientDomain -> Integers` (strong Gröbner basis over Z) and
  `RationalFunctions` (basis over the field Q(params)): recognised but
  currently computed over the rationals.
