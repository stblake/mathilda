...
| `CoefficientDomain`  | `Rationals`      | `Rationals`, `Automatic`, `RationalFunctions` (basis over the field Q(params)), `Polynomials[x, ...]` (named symbols become coefficient-ring parameters), `InexactNumbers[p]` (monic basis with `p`-digit real coefficients) |
...
- `CoefficientDomain -> Integers` (strong Gröbner basis over Z):
  recognised but currently computed over the rationals.
