...
Represents an element of a rational extension field in the power basis of a
generator.

- `AlgebraicNumber[theta, {c0, c1, ..., cn}]` — the algebraic number
  `c0 + c1 theta + ... + cn theta^n` in the field `Q(theta)`.

The generator `theta` may be given as a rational, a radical, a `Root` object, or
another `AlgebraicNumber`; the coefficients `ci` must be integers or rationals.
...
polynomial of `theta` (short lists are zero-padded, over-length lists fold
modulo the minimal polynomial). Objects that represent a rational number reduce
to explicit rational form. `AlgebraicNumber` carries `{NHoldAll, Protected}` and
is treated as a numeric quantity: `N` gives its value to any precision,
`RootReduce` converts it to a `Root` object, and `Re`, `Im`, `Abs`, `Round`,
`Less`, `Equal`, `NumericQ` operate on it. `AlgebraicNumber` objects in the same
field (same generator) are combined by `+`, `*`, `/`, and integer powers.

Backed by FLINT's `qqbar` engine; without FLINT the head is inert. `Re`/`Im`
return the object itself (or `0`) only for a real algebraic number.
...
In[2]:= 1 + %^2
Out[2]= AlgebraicNumber[Root[1 + #1 + #1^3 &, 3], {-2, -1, 5}]

In[3]:= AlgebraicNumber[(1 + I)/2, {1, 3}]            (* generator -> algebraic integer *)
Out[3]= AlgebraicNumber[1 + I, {1, 3/2}]

In[4]:= AlgebraicNumber[3^(1/5), {1, 2, 1, 3, 3, 1}]  (* fold over the minimal polynomial *)
...
In[6]:= N[AlgebraicNumber[Sqrt[2] I, {1, -1}], 50]
Out[6]= 1.0000000000000000000000000000000000000000000000000 -
          1.4142135623730950488016887242096980785696718753769 I
```

## ToNumberField

Expresses algebraic numbers as `AlgebraicNumber` objects in a named or common
number field.

- `ToNumberField[a, theta]` — express `a` in `Q(theta)`; unevaluated if
  `a` is not in `Q(theta)`.
- `ToNumberField[{a1, ...}, theta]` — express each `ai` in `Q(theta)`.
- `ToNumberField[{a1, ...}]` (equivalently `..., Automatic`) — express the `ai`
  in a common extension field generated by a single algebraic number.
- `ToNumberField[{a1, ...}, All]` — as above, using the smallest common field.
- `ToNumberField[x]` — convert any algebraic number to an explicit
  `AlgebraicNumber` object.

The `ai` and `theta` may be rationals, radicals, `Root`, or `AlgebraicNumber`
...
In[3]:= ToNumberField[{Sqrt[2], I}, All][[1, 1]]     (* primitive element of Q(Sqrt[2], I) *)
...
Gives an integral basis for a number field generated by an algebraic number.

- `NumberFieldIntegralBasis[a]` — a list of algebraic integers generating the
  ring of integers `O_K` of the field `Q(a)` as a module over the integers.

The number of elements equals the degree `[Q(a) : Q]`, and the first element is
always `1`. Each element is returned as a canonical `AlgebraicNumber` object (or
a plain integer/rational), so it displays symbolically; apply `N` for the
numeric approximations shown below. `a` may be a rational, radical, `Root`, or
`AlgebraicNumber` object; when `Q(a) = Q` the basis is `{1}`.

The algebraic-integer generator `phi` of `Q(a)` is formed first (rescaling `a`
when it is not itself an algebraic integer), then the equation order `Z[phi]` is
enlarged to the maximal order `O_K` by **Round 2 (Pohst–Zassenhaus)** wherever it
is not already maximal, so the basis is correct for non-monogenic fields (e.g.
`Q(Sqrt[2] + Sqrt[3])`, or `Q(Sqrt[5])`, whose ring of integers is
`Z[(1+Sqrt[5])/2]`, not `Z[Sqrt[5]]`). The basis produced is not unique. The
head is left unevaluated (after a `NumberFieldIntegralBasis::nintbas` message)
when `a` is not a constant algebraic number, its degree exceeds the cap, or the
maximal order cannot be certified (a discriminant that will not factor, or a
prime too large for a single-word modulus).

Attributes: `{Listable, Protected}` — `NumberFieldIntegralBasis[{a1, ...}]`
threads to one basis per generator (each is its own field). Requires FLINT;
backed by the `qqbar` engine and the number-field maximal-order layer.

```
In[1]:= N[NumberFieldIntegralBasis[2^(1/3)]]           (* Q(2^(1/3)), monogenic *)
Out[1]= {1., 1.25992, 1.5874}

In[2]:= N[NumberFieldIntegralBasis[Sqrt[2] + Sqrt[3]]] (* Z[theta] not maximal *)
Out[2]= {1., 3.14626, 5.44949, 13.3706}

In[3]:= N[NumberFieldIntegralBasis[E^(I Pi/4)]]        (* Z[zeta_8] *)
Out[3]= {1., 0.707107 + 0.707107 I, 1. I, -0.707107 + 0.707107 I}

In[4]:= AlgebraicIntegerQ /@ NumberFieldIntegralBasis[E^(I Pi/4)]
Out[4]= {True, True, True, True}

In[5]:= N[NumberFieldIntegralBasis[{Sqrt[2], Sqrt[5]}]]  (* threads over lists *)
Out[5]= {{1., 1.41421}, {1., 1.61803}}
```

## AlgebraicIntegerQ

Tests whether a number is an algebraic integer.

- `AlgebraicIntegerQ[x]` — `True` if `x` is a root of a monic polynomial with
  integer coefficients, `False` otherwise.

Decided exactly (no numeric oracle): `x` is an algebraic integer iff its
primitive integer minimal polynomial is monic. Rational integers qualify;
non-integer rationals, non-algebraic quantities (e.g. `Pi`), symbols, and lists
do not (`AlgebraicIntegerQ` is `Protected`, not `Listable`, so a list yields
`False` — use `/@` to test elements). Requires FLINT; a `USE_FLINT=0` build
leaves the head unevaluated.
...
In[6]:= Apart[1/((Sqrt[x] + 2) (Sqrt[x] + 3)), x]
Out[6]= 1/(2 + Sqrt[x]) - 1/(3 + Sqrt[x])
```
