...
| Method | Solves |
|---|---|
| `DSolve`Factorable` | factors the equation as a product in the derivatives (`F1·F2·…==0`) and recurses the cascade on each *differential* factor, unioning the branch bodies (e.g. `(y'-y)(y'+y)==0`). Factors over plain-symbol substitutes under a `PolynomialQ` gate; keeps only factors containing a derivative (the trivial `y==0` factor is dropped). Runs at the front |
| `DSolve`NthAlgebraic` | algebraic of degree ≥ 2 in the top derivative `y^(n)`: `Solve` for `y^(n)`, recurse each branch (a branch free of `y` hits `Quadrature`), union — e.g. `(y')^2 == 4 y → (x+C[1])^2`, `(y')^2 == 2`; plus the degenerate no-derivative case. Runs at the front |
...
| `DSolve`AbelAIR` | `y'[x] == N(x,y)/D(x,y)` with `D = c(x)(P1 y+P0)^k` a single linear y-factor (k∈{1,2}) — the Abel-2nd-kind / rational-in-y forms the polynomial-RHS specialists reject; scaling `u = y/s` (`s = -P0/P1`) makes it separable → **implicit** first integral. A partial Abel Invariant Rational method (the full rational-invariant solvable-class table is deferred); solves e.g. `y'+y == 2x E^-x/(1+E^x y)` |
...
| `DSolve`SolvableForY` | the **"dp" differentiation method** (Maple's `dp`), the general form of which `DSolve`Lagrange` is the linear special case. For an ODE `F(x,y,y')==0` that is **polynomial in `y`** (`PolynomialQ` gate) but not caught by an earlier specialist, isolate `y = G(x,p)` (`p=y'`), differentiate w.r.t. `x` (`p = G_x + G_p p'`), and recurse the cascade on the induced first-order ODE `dx/dp = G_p/(p − G_x)` for `x = X(p,C)`; the general solution is **parametric** `{{x->Function[{p},X], y->Function[{p},G(X,p)]}}`, back-substitution verified. Solves the 12000.org `y=_G(x,y')` class, e.g. `x^(n-1)(y')^n − n x y' + y == 0` (`y = n x y' − x^(n-1)(y')^n` is not affine in `x`, so Lagrange declines). Declines the Clairaut/singular case `p − G_x == 0` (owned earlier), a `G` free of `p`, and a `Root[…]`/implicit solve branch. Runs late in the first-order cascade (after every named specialist, before the Lie backstop); bounded (`TimeConstrained` sub-solves + wall-clock deadline + re-entry guard + decline memo); declines an IVP (parametric fitting is future) |
| `DSolve`SolvableForX` | the `x`-mirror of `DSolve`SolvableForY`: isolate `x = H(y,p)` (`dx/dy = 1/p`) and recurse on `dy/dp = H_p/(1/p − H_y)` for `y = Y(p,C)`, then `x = H(Y,p)`. **Pinned-only** (opt-in, not in the automatic cascade — its automatic corpus yield is ~0 while a degree-1-in-`x` form with a cubic-denominator induced ODE produces a transcendental branch whose back-substitution verify is slow), matching `FirstOrderPowerSeries` / `EigenvalueProblem`. Solves e.g. `x − y y' − (y')^2 == 0` |
...
| `DSolve`PiecewiseForcing` | linear ODE **IVP** with **piecewise / step / Heaviside** forcing (`Piecewise[…]`, `UnitStep[…]`), by **interval continuation** — no `LaplaceTransform` needed. Normalize the forcing's finite breakpoints `t0<b1<…<bm`; solve interval 0 with the given ICs (recursing the cascade), read off the `C^(n-1)` continuity data `y,…,y^(n-1)` at each breakpoint as the next interval's ICs, and assemble a back-substitution-verified `Piecewise` (Mathematica's own form). E.g. `y''+4y==Piecewise[{{1,0<=t<Pi}},0], y(0)=1,y'(0)=0 → Piecewise[{{1/4+3/4 Cos[2t], t<Pi},{Cos[2t], t>=Pi}},0]`. Runs before `UndeterminedCoefficients`/`LinearConstantCoefficients` (whose VoP would leave an inert `Integrate[UnitStep[…]·Cos,t]`), gated to step-forced linear IVPs; carries its own per-interval + per-IC numeric verify |
...
| `DSolve`SpecialFunctionForm` | 2nd-order forms → **Legendre / associated Legendre** (`(1-x²)y''−2x y'+(ν(ν+1)−μ²/(1-x²))y==0` → `LegendreP[ν,(μ,)x]`, `LegendreQ[ν,(μ,)x]`, e.g. `(1-x²)y''−2x y'+(15/4)y==0 → LegendreP[3/2,x]`, `LegendreQ[3/2,x]`), Airy (`y''==(Ax+B)y`), Bessel / modified Bessel, the **Bessel-reducible pure-power potential** `y'' + A xᵐ y == 0` (`m` numeric OR **symbolic**, ≠ 0, −2) → `Sqrt[x] Z_{1/(m+2)}((2√A/(m+2)) x^{(m+2)/2})` with `Z = J/Y` (e.g. `y''−x⁴y==0 → Sqrt[x](C[1] BesselI[1/6, x³/3] + C[2] BesselK[1/6, x³/3])`; a symbolic `m` — e.g. `y''+x^n y==0` — previously fell to the series fallback and now returns the closed Bessel form), the **two-term power potential** `y'' + (α x^{2c} + β x^{c-1}) y == 0` — NOT Bessel but confluent: under `ξ = x^{c+1}` it becomes a Coulomb equation, solved by `x^{(1-d)/2} WhittakerM[κ, ±μ, z]` (`d = c+1`, `μ = 1/(2d)`, `κ = β/(2 d √(-α))`, `z = (2√(-α)/d) x^d`) emitted as the verifiable `Exp[-z/2] z^{1/2±μ} Hypergeometric1F1[1/2±μ-κ, 1±2μ, z]` (its `y'`-carrying siblings `y''+a xⁿ y'+b x^{n-1} y==0` reach it through the Liouville normal-form pre-pass; both numeric self-verified), the **exponential-argument Bessel** `y'' + A e^{λx} y == 0 → Z_0((2√\|A\|/λ) e^{λx/2})` (e.g. `y''−e^{5x}y==0 → BesselI[0,(2/5)e^{5x/2}]`, `BesselK[0,·]`), the **normal-form Bessel** `y'' + (A + B/x²) y == 0 → Sqrt[x] Z_{√(1/4−B)}(√\|A\| x)`, Kummer confluent hypergeometric (`x y''+(b-x)y'-a y==0` → `Hypergeometric1F1`) and Gauss hypergeometric (`x(1-x) y''+(c-(a+b+1)x)y'-a b y==0` → `Hypergeometric2F1`); the hypergeometric second solution is emitted whenever the exponent parameter (`b` resp. `c`) is not a **provable integer** — including a symbolic parameter (e.g. symbolic `a,b,c` give both `Hypergeometric2F1` solutions). Any rational-coefficient equation with two finite regular singular points `{x1,x2}` off `{0,1}` is mapped affinely onto `x(1-x)` and its local exponents pulled out via `Y = s^r0 (1-s)^r1 F`, reaching **`Hypergeometric2F1`** for **Gegenbauer / Jacobi / associated Legendre at symbolic degree/order** (numeric self-verified; ordinary Legendre keeps `LegendreP`). An equation carrying a `y'` term is also tried through its **Liouville normal form** `z''=r z` against the Airy/Bessel recognisers. **Confluent / Whittaker** (y'-free `P==0` surface): a `y''+Q y==0` whose `Q` has ONE finite regular singular point (a double pole) and a rank-1 irregular point at infinity (`Q → b2 ≠ 0`) is Whittaker's equation, emitted as `Exp[-z/2] z^(1/2±μ) Hypergeometric1F1[1/2±μ-κ, 1±2μ, z]` (`z=2√(-b2)(x-x0)`, `μ=√(1/4-b0)`, `κ=b1/(2√(-b2))`; e.g. `x²y''+(c x²+b x+a)y==0`), self-verified against the reduced equation and declining when `2μ` is an integer (dependent partners). The `P≠0` confluent family (with a recovery factor) is future work |
| `DSolve`Kovacic` | 2nd-order linear `y'' + P y' + Q y == 0` → Liouvillian solutions via the reduced form `z'' == r z`: **Case 1** (rational `ω`, `z == P Exp[∫ω]`) as the *full* classical algorithm — `ω = s(∞)[√r]_∞ + Σ_c(s(c)[√r]_c + α_c^{s(c)}/(x−c))` with the local `[√r]_c` (even-order poles ≥ 4 supported) and, when `r` grows at ∞ (`O(∞) = degd−degn ≤ 0`), the non-constant polynomial part `[√r]_∞`; degree bound `d = α_∞^{s(∞)} − Σ_c α_c^{s(c)}`, monic `P` solving `P''+2ωP'+(ω'+ω²−r)P==0`. Prefers two independent `P·Exp` families directly (e.g. `y''−(x²+3/(4x²))y==0 → x^(−1/2)(C[1]e^{−x²/2}+C[2]e^{x²/2})`), else reduction of order; solves the **Legendre / Chebyshev / Gegenbauer / Jacobi** family and completely-reducible higher-pole / growth-at-∞ equations in elementary form. A denominator-degree gate declines high-degree `r` up front (no unbounded Factor/Solve), and a reduction-of-order integral in a hang-prone special function (`Erf`/`Erfi`/…) is declined rather than Simplify'd. **Case 2** (degree-2 algebraic, numeric-verified; skipped when a denominator factor is degree ≥ 2, to avoid a σ-solve blow-up); declines otherwise (non-elementary 2nd solution, or Case 3). An **inhomogeneous** equation `y''+Py'+Qy == g(x)` is closed by variation of parameters over the recovered fundamental set — first **de-obfuscated** (`Exp[c Log u]→uᶜ`, radicands split via `FactorList`, `PowerExpand`, so e.g. the Legendre-type `(x²−1)y''−2xy'+2y == x²−1`'s hidden `{x, (x−1)²}` basis surfaces and the VoP integral closes) — then re-verified numerically |
| `DSolve`SymmetricSquare` | homogeneous linear of order 3 whose solution space is the **symmetric square** of a 2nd-order equation: depress `y'''+p2 y''+p1 y'+p0 y==0` to `w'''+P1 w'+P0 w==0` (closed-form `P1 = p1−p2²/3−p2'`, `P0 = p0−p1 p2/3+2p2³/27−p2''/3`), test the signature `P0 == P1'/2`, solve the underlying `u'' + (P1/4) u == 0` (Airy/Bessel/…), and return `C1 u1² + C2 u1 u2 + C3 u2²` times the depression factor. Runs before `OperatorFactor`. E.g. `y'''−4(x+2)y'−2y==0 → AiryAi[2+x]²`, `AiryAi[2+x]AiryBi[2+x]`, `AiryBi[2+x]²`; the Bessel case gives Bessel products |
| `DSolve`OperatorFactor` | homogeneous linear of order ≥ 3: factor the operator by finding a first-order right factor `(D − r)`, `r ∈ C(x)` (a hyperexponential solution `Exp[∫r]`, via a rational Riccati `Σ a_k P_k(r) == 0` search), peel it via operator right-division, recurse on the order-(n−1) quotient, and close with one first-order linear solve. Solves reducible variable-coefficient operators the earlier methods miss (e.g. shifted-Euler at a pole ≠ 0). Declines when no rational first-order factor exists. A pole-count gate declines many-pole ansätze up front (the cubic determining system would otherwise not terminate) |
| `DSolve`DFactor` | `DSolve`DFactor[eqn, y, x]` returns the operator factorization `{Dx − r1, Dx − r2, …}` (`Dx` an inert d/dx operator; factors compose left-to-right, `L = (Dx−r1)(Dx−r2)…`); a not-fully-reducible operator returns its first-order factors plus an inert remainder operator `Σ q_j Dx^j` |
| `DSolve`FirstOrderSubstitution` | `y'[x] == F(a x + b y + c)` (substitution `v = y + (F_x/F_y) x` → autonomous separable) |
| `DSolve`PolynomialShiftSubstitution` | `y'[x] == R(x) + g(x)(φ(x)+c y)^p` (`p` non-integer, `φ` polynomial in x, `c` constant, `R = −φ'/c`): the x-dependent-shift generalisation of `FirstOrderSubstitution`. `u = φ(x)+c y` reduces it to the separable `u' == c g(x) u^p`; returns the **implicit** first integral `{{u^(1−p)/(1−p) − ∫c g dx == C[1]}}` (branch-safe; verified by the implicit-function rule). The deterministic replacement for the radical `[F(x),G(x)]`-symmetry cases the Lie `abaco2_similar` heuristic aborts on, e.g. `y' == x²/2 + (1+x²+x³)Sqrt[x³−6y]` → `2Sqrt[x³−6y] + 6x + 2x³ + 3x⁴/2 == C[1]` |
| `DSolve`LinearCoefficients` | `y'[x] == (a₁x+b₁y+c₁)/(a₂x+b₂y+c₂)`: det≠0 shifts to the lines' intersection → `Homogeneous` (explicit Root-form, or implicit log-spiral first integral); det=0 (parallel) substitutes `v=a₁x+b₁y` → separable. Deterministic, earlier than the Lie `linear` heuristic |
| `DSolve`AlmostLinear` | `f(x)g(y)y' + k(x)l(y) + m(x)==0` — substitution `u=Integrate[g,y]` linearises it to `u'+P(x)u==Q(x)` (integrating factor), then `l(y)==U(x)` solved for `y` (e.g. `2 y y' + y² − x == 0`) |
| `DSolve`SeparableReduced` | `x y'/y == G(x^n y)` — power `n = x r_x/(y r_y)` (constant iff of this form), substitution `w = x^n y` → separable; **implicit** first integral (e.g. `y' == y²/(1 + x y)`) |
| `DSolve`LieSymmetry` (alias `DSolve`LieGroup`) | first-order `y'[x] == ω(x,y)` via a **Lie point symmetry** `V = ξ ∂ₓ + η ∂_y`: a table of ansatz *heuristics* proposes `(ξ,η)`, each gated by the linearized symmetry condition `η_x + (η_y − ξ_x)ω − ξ_y ω² − ξ ω_x − η ω_y == 0`; the accepted symmetry gives the Lie integrating factor `μ == 1/(η − ξ ω)`, whose exact-equation quadrature is the **implicit** first integral `{{G(x,y[x]) == C[1]}}` (verified by implicit differentiation). The general first-order backstop, run after the deterministic specialists and before the series fallback. The only heuristic method in the cascade — first-order symmetry finding is an underdetermined PDE. Heuristics implemented (tried cheapest-first): `abaco1_simple` (one-variable ansätze), `linear` (affine ansatz — the **linear-coefficients** class `y'==(a1 x+b1 y+c1)/(a2 x+b2 y+c2)`, solved via the determining-system `NullSpace`, e.g. `y'==(x+2y-4)/(2x+y-5)`), `abaco1_product` (the product ansatz `[ξ=F(x)G(y), η=0]` and its inverse `[0, F(x)G(y)]` — a rational-but-non-polynomial infinitesimal such as `ξ=y/x` that the polynomial ansätze miss, e.g. `y'==2xy/(x²+2y⁴+2)`; via the Cheb-Terrab & Roche Eq-19 product-separability), `function_sum` (the additive ansatz `[ξ=F(x)+G(y), η=0]` and its inverse `[0, F(x)+G(y)]` — §4.2, from the rational factor `ω·∂²ₓ(1/ω)=F''/(F+G)` whose reciprocal separates by product), `abaco2_similar` (the similarity ansatz `[ξ=F(x), η=H(x)]` and its inverse `[F(y), H(y)]` — the first to reach **irrational** `ω`, from `Q=ω_y/ω_yy`, `T=Q_x/Q_y` free of `y`, `F=Exp[∫((T ω_y − T_x − ω_x)/(ω+T)) dx]`, `H=−T F`; solves `y'==Sqrt[a x+b y+c]` and `(a x+b y+c)^p`, `p` non-integer, via Cheb-Terrab & Roche §4.3), `abaco2_unique_unknown` (the ansätze `[ξ=F(x), η=G(y)]` and `[ξ=G(y), η=F(x)]` — §4.4.1, from a function or non-integer power of both variables in `ω`: for each such mapping `M`, `R=M_y/M_x`, and when `R` separates by product with x-factor `X` the candidates `[X, −X/R]`/`[−R/X, 1/X]` are gated; solves `y'==(x/y)(x²+y²)^(1/3)`; plus the §4.4.1 "differential invariant of order zero" extension — the non-separable candidates `[−R,1]`/`[1,−R]`/`[1,−1/R]`, catching Kamke ODE 433 `(x y'+y+2x)²==4(x y+x²+a)` → `x−Sqrt[x²+x y+a]==C[1]`), and `bivariate` (general degree-2/3 bivariate-polynomial ansatz — the same `NullSpace` determining system at higher degree, catching genuinely quadratic/projective symmetries the affine case misses, e.g. `ξ=x², η=x y` for `y' == -1/x + y/x + y²/x³`), and `chi` (CPC 101 1997, 5th algorithm — the `η==ξω+χ` reformulation: a rich-basis ansatz for `χ` in the transcendental **atoms** of `ω` (`Sin/Cos`, `Sinh/Cosh`, `Log`, inverse trig) over their product, cleared + reduced modulo the Pythagorean relations → `NullSpace` determining system; the one heuristic whose `χ` may be a genuine transcendental beyond `bivariate`'s polynomial reach, e.g. Kamke ODE 357 `x y' ln(x)sin(y)+cos(y)(1−x cos(y))==0` → `−x+Log[x]Sec[y[x]]==C[1]`). An `ω` carrying an undefined function of both variables (`Tan[ArcTan[y]+F[x²+y²]]`) declines promptly rather than hanging (a node budget on the hot-path helpers + a polynomial zero-test + an undefined-function gate). The formal §4.4.2 Case I/II (`abaco2_unique_general`) is a documented exemption (impractical per the source paper). See `docs/design/dsolve_lie_symmetry.md` |
| `DSolve`ReductionOfOrder` | 2nd-order missing `y`, `y'' == F(x, y')` (reduce to 1st order in `p = y'`, then integrate) |
| `DSolve`LowerDerivativeReduction` | order ≥ 3 with `y` and every derivative below some order `m ≥ 1` absent: set `p = y^(m)` (order drops by `m`), solve the order-`(n−m)` equation, integrate `p` back `m` times. E.g. `7 y' y''' − 11 y''² == 0 → C3 − C2/(3(4x+7C1)^{3/4})` (via `7 p p'' − 11 p'² == 0`) |
| `DSolve`AutonomousReduction` | autonomous ODE of **any order n ≥ 2**, `y⁽ⁿ⁾ == f(y, y', …, y⁽ⁿ⁻¹⁾)` missing `x`: the reduction `p = y'(y)` (derivative chain `D_{k+1}=p·d/dy(D_k)`) gives an order-(n−1) ODE in `p(y)`, solved by recursion, then `y' == p(y)` separable; stage-1 constants `C[1..n−1]` frozen to `C[2..n]` before stage 2. Order 2 is the classical `p p_y == f` (`y y''==(y')² → C E^(C x)`); order 3 solves e.g. `y y'''==y'y''`. A stage-2 spin-guard declines when the separable quadrature `∫dy/p` is non-elementary (a `Log`- or y-denominator radicand, where Integrate spins uninterruptibly); those are instead picked up by the **implicit companion (M59)**, which returns the inert first integral `Inactive[Integrate][1/p, y[x]] − x == C[1]` (built and verified without running Integrate, so no spin — e.g. the order-3 autonomous `y y'''==(x²yy'+…)` family). The explicit-then-implicit method is wired so an elementary-quadrature case (`y y'''==y'y''`) still gets the full explicit solution first |
| `DSolve`Liouville` | `y'' + g(y)(y')² + h(x)y' == 0` (both `y` and `x` present, so the missing-`y`/missing-`x` reductions decline) by two quadratures: `Integrate[Exp[Integrate[g,y]],y] == C[1] Integrate[Exp[-Integrate[h,x]],x] + C[2]`, solved for `y` (e.g. `y'' + (y')²/y + y'/x == 0`) |
| `DSolve`ChangeOfVariable` | 2nd-order **linear** with **transcendental** coefficients `y''+P(x)y'+Q(x)y==0`: a change of the independent variable `t=φ(x)` (`Cos`/`Sin`/`Tan`) that rationalizes the coefficients (`A=(φ''+Pφ')/φ'²`, `B=Q/φ'²` become rational in `t`), then the transformed equation is handed back to the cascade and the solution composed with `t=φ(x)`. E.g. `y''+Cot[x]y'+k(k+1)y==0 --(t=Cos[x])--> (1−t²)Y''−2tY'+k(k+1)Y==0` (Legendre). NUMERICALLY verified on the original (Legendre-Q carries `Log`); bounded sub-solves + re-entry guard; declines already-rational equations |
| `DSolve`VariationOfParameters` | general nonhomogeneous backstop `L[y]==g(x)` (`g≢0`) with **transcendental** coefficients — the `ChangeOfVariable` family, whose homogeneous part is solved by a method that does not itself carry forcing. Recurses `DSolve` on `L[y]==0`, normalises the fundamental set (`PowerExpand[Simplify[·]]`, reducing residual radicals such as `1/Sqrt[Sec²x]→Cos x` so the VoP integrals close), adds the particular by variation of parameters, returns `hom+yp` (numerically self-verified; declines on an inert `Integrate`). Runs after `SpecialFunctionForm`/`Kovacic`/`ChangeOfVariable`, so it is a pure backstop (never preempts them); gated to trigonometric coefficients (rational cases are Kovacic's/Euler's domain). E.g. `Sin[x]y''+(2Sin−Cos)y'+(Sin−Cos)y==E^-x → −E^-x Sin[x] + {E^-x, E^-x Cos[x]}` |
| `DSolve`ReducibleIntegratingFactor` | general **nonlinear** 2nd-order `y'' == Φ(x,y,y')` via an **integrating factor** `μ` of a restricted form (Cheb-Terrab & Roche 1999; Maple `_reducible, _mu_*`): `μ(y''−Φ)` a total x-derivative `dR/dx` gives `μ=R_{y'}`, `R=∫μ dy'+G(x,y)` with `G` from `R_x+y'R_y+Φ R_{y'}==0`; then `R(x,y,y')==C[1]` is a **first-order ODE** solved by recursing the cascade. Stage 1 = `μ(x,y)`: Φ a degree-≤2 polynomial in `y'` (`y''=a y'²+b y'+c`), branch on `2a_x−b_y` — Case A closed-form `μ`, Case B `μ=ν(x)e^{−∫a dy}` with `ν` from one linear ODE (e.g. `y y'+y''==1`, `y''−y y'==6` → Airy; `−(y'²/y²)+y''/y+2Coth[2x]y'/y==2`). Two verify gates (symbolic first-integral test `R_x+y'R_y+Φ R_{y'}==0` + numeric back-substitution) mean a wrong `μ` yields a clean decline, never a wrong answer; a **linearity gate** declines linear ODEs; `TimeConstrained`-bounded sub-solves + deadline + decline memo. Runs before `SecondOrderSymmetry` (algebraic μ-search is cheaper and reaches ODEs with no point symmetry). Stage 2 = `μ(x,y')` (Section 2.2, Lemma 3): 𝓕 from the y'-only factors of `Υ=Φ_y` (Case A), the y-only factors + `p'=𝓗_x/𝓗_y` (Case C), or `Υ_y=0` with `p'=Ψ_x` (Case D), and `μ̃(x)` by Lemma 2 — solves e.g. Kamke 226 `y''=(x²yy'+xy²)/y'` (μ=y'), Kamke 136 `y''=(1+y'²)/(x−y)`, Kamke 66 `y''=a(c+bx+y)(1+y'²)^{3/2}` |
| `DSolve`ReducibleFirstIntegral` | the **reduction-of-order** companion of `DSolve`ReducibleIntegratingFactor`: finds the same integrating factor `μ` and reconstructs the first integral `R(x,y,y')` (gated by the exactness test `R_x+y'R_y+Φ R_{y'}==0`, symbolic + numeric), then returns it as the implicit relation `{{ R(x, y[x], y'[x]) == C[1] }}` — used when that first-order ODE has **no elementary explicit solution** (Maple-style, e.g. Kamke 226 → `y'[x]²/2 − x²y[x]²/2 == C[1]`). Runs in the automatic cascade **after** the full-solution 2nd-order methods (`ReducibleIntegratingFactor`-explicit and `SecondOrderSymmetry`), so a two-constant closed form always wins over the one-constant reduction; declines an IVP (one constant cannot fit two conditions) |
| `DSolve`SecondOrderSymmetry` | general **nonlinear** 2nd-order `y'' == Φ(x,y,y')` via a Lie **point** symmetry `X=ξ∂ₓ+η∂_y`: a polynomial-ansatz determining system (2nd-prolongation `NullSpace`, the first-order Lie machinery lifted to `{x,y,p}`) finds `(ξ,η)`, then canonical coordinates `(r,s)` reduce the order to a first-order `dq/dr==F(r,q)` (solved by recursing the cascade), integrated to `s==S(r)+C[2]` and inverted for `y`. Every inversion branch is **numerically** back-substitution verified (a wrong branch with a symbolically-undecidable residual would else slip through `dsolve_run`'s symbolic verify); linear ODEs are declined (Euler/Kovacic/SpecialFunction/Frobenius own them); all recursive sub-solves are `TimeConstrained`-bounded + a wall-clock deadline + a decline memo, so a decline is a clean bounded fall-through. Solves the scaling/projective/`_mu_*`-reducible Kamke/Murphy families (e.g. `x³y''==(y−xy')²`, `x²yy''+x²y'²−5xyy'==4y²`, `x²(x+y)y''==(xy'−y)²`) |
| `DSolve`FrobeniusSeries` / `DSolve`PowerSeries` | truncated series about `x == 0` (the always-available fallback, tried last): ordinary point → two power series; regular singular → Frobenius `x^s Σ a_n x^n` (indicial quadratic `s(s-1)+p₀ s+q₀`, with a `Log` term for equal roots). The pinned method keeps its "series about the origin, else decline" contract; **automatic** `DSolve`, as a very last resort, expands a *rational*-coefficient linear ODE about a nearby ordinary point (`SeriesData[x, x0, …]`) when `x == 0` is irregular/obstructed, so such an equation never returns unevaluated |
| `DSolve`FirstOrderPowerSeries` | truncated power series about the ordinary point `x0 == 0` for a first-order `y'[x] == F(x, y)` (a₀ == C[1]; `[xⁿ]F` depends only on a₀..aₙ, one Taylor read per order) — e.g. the nonlinear `y' == x + y²`. **Pinned-only** (not in the automatic cascade): a first-order ODE with no closed form stays unevaluated by default, matching SymPy's opt-in `1st_power_series` and Mathematica |
...
| `DSolve`LinearFirstOrderSystem` | `Y' == A Y + b(x)`, constant `A`, **any** spectrum — fundamental matrix `e^{Ax}` from `JordanDecomposition` (diagonalizable, **defective**, or complex); forcing by variation of parameters. A **real-irrational** eigenvalue is abstracted to a fresh symbol before the VoP integral (else `Integrate` rationalises the `1/λᵏ` coefficient into a hundreds-of-digit integer and spins — `{x'=2x+4y+3eᵗ, y'=5x−y−t²}`, `λ=(1±√89)/2`, ran >90 s), then substituted back; complex and rational spectra stay concrete (resonance-safe, and the complex real-trig path is untouched) |
...
| `DSolve`PDEQuasilinear` | first-order **quasilinear** `P u_{v1} + Q u_{v2} == R` (linear in the first derivatives; `P, Q, R` may depend on `u`) by **Lagrange's** method of characteristics `dv1/P = dv2/Q = du/R`. Two tractable classes: **semilinear** (`P, Q` free of `u`, `R` affine in `u`) — the base characteristic `dv2/dv1 = Q/P` decouples (recurse the scalar cascade for its invariant `ξ`), giving the **explicit** general solution with an arbitrary function `C[1][ξ]` (e.g. `x u_x + y u_y == u → x C[1][y/x]`; generalizes `PDELinearFirstOrder` to variable coefficients); **conservation law** (`R == 0`, coefficients touching `u`) — `u` is invariant along characteristics, giving the **implicit** relation `φ1(v1,v2,u) == C[1][u]` (inviscid Burgers `u u_x + u_y == 0`, verified by implicit differentiation). Declines coefficients depending on `u` with `R ≠ 0`, and non-elementary characteristic integrals |
| `DSolve`PDEClairaut` | the first-order PDE **Clairaut** form `u == v1 u_{v1} + v2 u_{v2} + f(u_{v1}, u_{v2})`; the complete integral is the two-parameter plane family `u == C[1] v1 + C[2] v2 + f(C[1], C[2])`. With `IncludeSingularSolutions -> True` the singular integral (the envelope, from eliminating the constants of `{v1 + f_{C1} == 0, v2 + f_{C2} == 0}`) is also returned — e.g. `u == x u_x + y u_y + u_x u_y → C[1] C[2] + C[1] x + C[2] y` plus the envelope `-x y`. Nonlinear in the derivatives, so the linear/quasilinear methods decline to it (a degenerate linear `f` is a semilinear PDE, claimed by `PDEQuasilinear` first) |
| `DSolve`PDECharpit` | first-order **fully nonlinear** PDE `F(v1, v2, u, p, q) == 0` (`p = u_{v1}`, `q = u_{v2}`) for a complete integral, by **Charpit's** method restricted to the three classic standard forms: **`F(p,q)`** (free of `v1,v2,u`) → explicit `u == C[1] v1 + q v2 + C[2]`, `q` from `F(C[1],q)==0` (e.g. `p²+q²==1`); **`F(u,p,q)`** (free of `v1,v2`) → the **implicit** relation `Integrate[1/P, u] == v1 + C[1] v2 + C[2]`, `P` from `q==C[1] p` (e.g. `p q == u`); **separable `f(v1,p) == g(v2,q)`** (free of `u`) → explicit `u == Integrate[P,v1] + Integrate[Q,v2] + C[2]` (e.g. `p²−q²==v1−v2`). Runs after the quasilinear/Clairaut methods (it is nonlinear in the derivatives). Declines equations depending on all of `v1, v2, u` (the general integrable-combination search is future) and non-elementary integrals |
| `DSolve`PDELinearSecondOrder` | constant-coefficient 2nd-order linear PDE `A u_{v1 v1} + B u_{v1 v2} + C u_{v2 v2} + D u_{v1} + E u_{v2} + F u == 0` by operator factoring. **Principal part only** (`D=E=F=0`): the trial `u == f(v2 + λ v1)` gives the characteristic quadratic `A λ² + B λ + C == 0`, covering all three discriminant signs — distinct real roots (hyperbolic) → `C[1][v2+λ1 v1] + C[2][v2+λ2 v1]` (the wave equation `u_tt == c² u_xx → C[1][x-c t] + C[2][x+c t]`, d'Alembert); complex roots (elliptic) → the complex-characteristic form (Laplace `u_xx + u_yy == 0 → C[1][y-I x] + C[2][y+I x]`); a repeated root (parabolic) → `C[1][w] + v1 C[2][w]`, `w == v2 + λ v1`. **With lower-order terms**: when the full symbol factors into two first-order operators `(∂_v1 − λ_i ∂_v2 + m_i)`, each contributes `e^{−m_i v1} C[i][v2 + λ_i v1]` — the exponential-damped form, e.g. the distortionless telegraph `u_tt − c² u_xx + a u_t + (a²/4) u == 0 → e^{−a t/2}(C[1][x+c t] + C[2][x−c t])`, or the pure-mixed `u_xy + u_x == 0 → C[1][y] + e^{−y} C[2][x]`. Declines a non-factorable symbol (the general telegraph `b ≠ a²/4` needs Bessel functions), forcing, and non-constant coefficients |
...
The **initial-value problem** for the wave equation on the whole line is solved by
d'Alembert's formula — automatically, or via the pinned `DSolve`WaveDAlembert`:

```
In[19]:= DSolve[{D[u[x,t],{t,2}] == c^2 D[u[x,t],{x,2}], u[x,0] == f[x],
                 Derivative[0,1][u][x,0] == g[x]}, u, {x,t}]
Out[19]= {{u -> Function[{x, t}, (f[x - c t] + f[x + c t])/2
                 + Integrate[g[K], {K, x - c t, x + c t}]/(2 c)]}}
