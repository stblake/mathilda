...
| `DSolve`Kovacic` | 2nd-order linear `y'' + P y' + Q y == 0` → Liouvillian solutions via the reduced form `z'' == r z`: Case 1 (rational `ω`, `z == P Exp[∫θ]`) — including the classical apparent-singularity completion for rational `r` with genuine poles (a monic `P` over the local pole exponents `α_c = (1 ± √(1+4 b_c))/2`, poles located incl. complex via `dsolve_analyze_roots`), which solves the integer-degree **Legendre / Chebyshev / Gegenbauer / Jacobi** family in elementary form (e.g. `(1−x²)y''−2xy'+2y==0 → C[1] x + C[2](1 − x ArcTanh[x])`) — and Case 2 (degree-2 algebraic, numeric-verified; skipped when a denominator factor is degree ≥ 2, to avoid a σ-solve blow-up); declines otherwise |
