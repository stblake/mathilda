...
| `DSolve`SpecialFunctionForm` | 2nd-order forms → Airy (`y''==(Ax+B)y`), Bessel / modified Bessel, the **Bessel-reducible pure-power potential** `y'' + A xᵐ y == 0` (`m` a number ≠ 0, −2) → `Sqrt[x] Z_{1/(m+2)}((2√\|A\|/(m+2)) x^{(m+2)/2})` with `Z = J/Y` for `A>0` and `I/K` for `A<0` (e.g. `y''−x⁴y==0 → Sqrt[x](C[1] BesselI[1/6, x³/3] + C[2] BesselK[1/6, x³/3])`), Kummer confluent hypergeometric (`x y''+(b-x)y'-a y==0` → `Hypergeometric1F1`) and Gauss hypergeometric (`x(1-x) y''+(c-(a+b+1)x)y'-a b y==0` → `Hypergeometric2F1`); the hypergeometric second solution is emitted only when the exponent parameter (`b` resp. `c`) is a non-integer number |
