#!/usr/bin/env python3
"""Experiment 75 -- trust-krylov unconstrained optimization (scipy column).

Same 6 problems as ``trust_krylov.m``, same labels, same order, solved with
scipy.optimize.minimize(method="trust-krylov") and the analytic Jacobian AND
Hessian, so the race is solver-vs-solver (both Hessian-free GLTR/Lanczos
trust-region). The far-start Rosenbrock family has an indefinite Hessian en
route (handled by the exact tridiagonal solve); Trid has an integer optimum.
The check is the objective at the optimum rounded to 10^6 (0 for the f*=0
problems, the exact integer for Trid).
"""
