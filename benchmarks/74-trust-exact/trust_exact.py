#!/usr/bin/env python3
"""Experiment 74 -- trust-exact unconstrained optimization (scipy column).

Same 6 problems as ``trust_exact.m``, same labels, same order, solved with
scipy.optimize.minimize(method="trust-exact") and the analytic Jacobian AND
Hessian, so the race is solver-vs-solver (both near-exact Moré-Sorensen
trust-region). The far-start Rosenbrock family has an indefinite Hessian en
route (handled directly by the exact subproblem); Trid has an integer optimum.
