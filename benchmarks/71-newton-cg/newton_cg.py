#!/usr/bin/env python3
"""Experiment 71 -- Newton-CG unconstrained optimization (scipy column).

Same 6 problems as ``newton_cg.m``, same labels, same order, solved with
scipy.optimize.minimize(method="Newton-CG") and the analytic Jacobian AND
Hessian, so the race is solver-vs-solver (both truncated-Newton, both
Hessian-based). The far-start Rosenbrock family has an indefinite Hessian en
route, which Newton-CG's CG truncates on; Trid has a constant positive-definite
