#!/usr/bin/env python3
"""Experiment 81 -- Dual Annealing global optimization (scipy column).

Same 9 problems as ``dual_annealing.m``, same labels, same order, solved with
scipy.optimize.dual_annealing using scipy's DEFAULT parameters (visit 2.62,
accept -5, initial_temp 5230, maxiter 1000) and a fixed seed, so the race is
DualAnnealing-vs-dual_annealing on the same algorithm. These are standard
bounded multimodal benchmarks -- Himmelblau, Booth, Beale, Rosenbrock,
six-hump camel, Ackley (2-D and 5-D), Styblinski-Tang, Rastrigin -- whose
global both systems reach and polish to agreement. The check is the objective at
the global optimum rounded to 10^6, matching the .m ``Round[10^6 First[...]]``.
"""
