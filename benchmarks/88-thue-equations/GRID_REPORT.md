...
| verdict | n | meaning |
|---|---|---|
| CORRECT | 278 | finite set matches PARI |
...
| UNVERIFIED | 2 | PARI rejected the form (perfect power / repeated factor) |

**Result: 0 bug(s).** Every form Mathilda solved matched PARI exactly.

## Oracle misses (Mathilda correct, PARI's `thue()` incomplete)

Adjudicated by independently checking each disputed point against `F(x,y)==m`: Mathilda's set is sound and strictly contains a genuine solution PARI missed. PARI `thue()` is fallible on some totally-imaginary fields.

- `grid-0032-n4` — `y^4 - 3*x*y^3 + 4*x^2*y^2 - 2*x^3*y + x^4 == 5` (Mathilda 2, PARI 0). pcoef `[1, -3, 4, -2, 1]`.

## Solve paths exercised: 278 CORRECT

A sample (the grid's value is that these are *machine-generated*, not chosen to look good):

| label | form == m | #sol |
|---|---|---:|
| grid-0002-n4 | `-3*y^4 + 2*x*y^3 - x^2*y^2 + 2*x^3*y - x^4 == 7` | 0 |
...
| grid-0008-n4 | `4*y^4 - 3*x*y^3 + 4*x^2*y^2 + 3*x^3*y + x^4 == 1` | 2 |
...
| grid-0024-n4 | `-2*y^4 + 2*x*y^3 - 2*x^2*y^2 - 3*x^3*y - x^4 == 1` | 0 |
