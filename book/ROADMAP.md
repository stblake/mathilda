...
| 2 | A Brief Introduction to Mathilda | `chapters/02-introduction.tex` | **Verified** (first pass) |

**Ch. 2 scope.** A whirlwind tour across the whole system, breadth over depth, meant
...
functions, and a first plot. **Deferred:** plotting the §2.5 NDSolve solution as a
...
## Chapter 3 — Mathematics in Mathilda (sections 3.1–3.7)

This is one chapter (`chapters/03-mathematics.tex`) whose sections are the individual
mathematical domains, each its own file under `chapters/math/` and its own campaign.

| # | Section | File | Status |
|---|---------|------|--------|
| 3.1 | Arithmetic | `chapters/math/arithmetic.tex` | **Verified** |
| 3.2 | Algebra | `chapters/math/algebra.tex` | Planned |
| 3.3 | Calculus | `chapters/math/calculus.tex` | Planned |
| 3.4 | Linear Algebra | `chapters/math/linear-algebra.tex` | Planned |
| 3.5 | Numerical Calculus | `chapters/math/numerical-calculus.tex` | Planned |
| 3.6 | Number Theory | `chapters/math/number-theory.tex` | Planned |
| 3.7 | Special Functions | `chapters/math/special-functions.tex` | Planned |

**§3.1 Arithmetic scope** (three subsections, per the original outline):
- **3.1.1 Machine-precision** — internal sizes/limits of machine ints and doubles;
  that Mathilda does **not** use significance arithmetic, with examples where the
  fixed-precision `double` model works cleanly.
- **3.1.2 Arbitrary precision** — how machine, GMP bignum, and MPFR computations mix
  and hand off; automatic promotion/demotion; `N[expr, prec]` and precision literals.
- **3.1.3 Interval arithmetic** — overview, classical examples, advanced uses, and a
  comparison with the ISO standard for interval arithmetic.

**§3.2 Algebra scope.** Expand/Factor/Together/Cancel/Apart; polynomials over ℚ and
...
**§3.3 Calculus scope** (per outline): 3.3.1 Derivatives, 3.3.2 Limits, 3.3.3 Series,
3.3.4 Residues, 3.3.5 Indefinite integration (the Risch story), 3.3.6 Definite
integration (contour methods, Mellin, symmetry). A large section — likely split into
several campaigns.

**§3.4 Linear Algebra scope.** Vectors/matrices as expressions; Dot/Det/Inverse; the
...
**§3.5 Numerical Calculus scope.** `ND`, `NIntegrate`, `NSum`/`NProduct`, `NDSolve`,
`NLimit`/`NSeries`, `FindRoot`; accuracy/precision goals; when and why to go numeric.

**§3.6 Number Theory scope.** Primes, factorization pipeline (Rho, P±1, ECM),
modular arithmetic, Diophantine equations (`Solve[..., Integers]`), continued
fractions.

**§3.7 Special Functions scope.** Gamma/Zeta/PolyGamma families, Bessel/Airy,
error functions, hypergeometric functions; exact values, symbolic identities, and
rigorous `acb` numerics via FLINT.

## The System (Chapters 4–8)

| # | Chapter | File | Status |
|---|---------|------|--------|
| 4 | Graphics | `chapters/04-graphics.tex` | Planned |
| 5 | Data Structures | `chapters/05-data-structures.tex` | Planned |
| 6 | Programming in Mathilda | `chapters/06-programming.tex` | Planned |
| 7 | Compilation and the Compiler | `chapters/07-compilation.tex` | Planned |
| 8 | Data I/O | `chapters/08-data-io.tex` | Planned |

**Ch. 4 Graphics scope.** `Plot`/`Plot3D`/`ListPlot`/`Graphics`/`Show` primitives,
...
**Ch. 5 Data Structures scope.** Expression trees (everything is an expression);
...
**Ch. 6 Programming scope** (per outline): pattern matching, procedural programming,
functional programming — the three paradigms Mathilda supports, and how they compose.

**Ch. 7 Compilation scope.** `Compile[]`, auto-compilation, the bytecode VM, the
compilable subset as a cliff, and how packed arrays and compilation combine.

**Ch. 8 Data I/O scope.** *Mostly not implemented yet in Mathilda* — write this last,
and only cover what actually exists; be explicit about what is not yet supported.

## The Project (Chapters 9–12)

| # | Chapter | File | Status |
|---|---------|------|--------|
| 9 | The Internals of Mathilda | `chapters/09-internals.tex` | Planned |
| 10 | The Development of Mathilda | `chapters/10-development.tex` | Planned |
| 11 | Contributing to Mathilda | `chapters/11-contributing.tex` | Planned |
| 12 | About the Author | `chapters/12-about-the-author.tex` | Planned |

**Ch. 9 Internals scope.** The pipeline (parser → evaluator → printer), the symbol
table, attributes, the pattern matcher, the rule engine — a reader's tour of
`SPEC.md`.

**Ch. 10 Development scope.** How Mathilda is built, including the AI-in-the-loop
workflow; the testing, benchmarking, and audit machinery (the packed-array/compile
gates).

**Ch. 11 Contributing scope.** Adapted from `docs/extending.md`: adding a builtin, a
module, a pattern, an operator; the coding standards and the C99 portability gate.

**Ch. 12 About the Author.** Short.
...
to Ch. 15 (Internals) for depth. Further appendices can follow the same pattern.
