...
\pair{02-introduction/numbers/2}{An integer plus a rational\index{number!rational} stays \emph{exact}: the
fraction $7/3$, not a decimal.}
\pair{02-introduction/numbers/3}{A rational plus a \emph{real} gives a real\index{number!real}. One
...
$\pi/4$.}
\pair{02-introduction/elementary-exact/7}{At an exact argument with \emph{no} closed\index{closed form}
...
\pair{02-introduction/special-exact/4}{By analytic continuation\index{analytic continuation}, the notorious
$\zeta(-1)=-\tfrac{1}{12}$.}
...
\pair{02-introduction/number-theory/5}{\B{ExtendedGCD} adds B\'ezout's coefficients\index{Bezout's coefficients@B\'ezout's coefficients}:
...
\pair{02-introduction/solve-exact/3}{A linear system in two unknowns\index{system of equations!linear}, uniquely solved.}
...
\begin{codepairs}
\pair{02-introduction/solve-region/1}{Each nested radical\index{radical!nested} is a
...
\pair{02-introduction/solve-region/2}{A sum of absolute values\index{absolute value}
over~$x$: the equation is satisfied not at isolated points but along the whole ray
$x\ge3$.}
\pair{02-introduction/solve-region/3}{Nested absolute values, even in~$x$: the
...
\begin{codepairs}
\pair{02-introduction/inequalities/1}{A quadratic inequality: the solution set is the
closed interval where the parabola lies on or below the axis.}
\pair{02-introduction/inequalities/2}{A quartic can cut the line into several
pieces---here a union of two open intervals.}
\pair{02-introduction/inequalities/3}{A rational inequality. The pole at $x=-2$ is
excluded automatically---\B{Reduce} keeps track of where the expression is undefined.}
\pair{02-introduction/inequalities/4}{The endpoints need not be rational: here they are
the exact surds $\pm\sqrt2$.}
\pair{02-introduction/inequalities/5}{A rational inequality whose numerator carries the
denominator's factors---$x^{3}-x=x(x^{2}-1)$---so after cancellation it collapses to the
plain linear condition $x\le2$.}
\pair{02-introduction/inequalities/6}{A \emph{radical}
...
\begin{codepairs}
\pair{02-introduction/inequalities/7}{$x^{2}+1>0$ holds for every real $x$---a one-line
proof that the expression is positive-definite.}
\pair{02-introduction/inequalities/8}{No real number makes $x^{2}+x+1$ negative (its
discriminant is negative), so the answer is \texttt{False}.}
\pair{02-introduction/inequalities/9}{The arithmetic--geometric-mean
...
\pair{02-introduction/inequalities/10}{With two variables the solution set becomes a
...
\pair{02-introduction/inequalities/11}{Absolute values\index{absolute value} in two
...
\begin{codepairs}
\pair{02-introduction/simplification/1}{The Pythagorean identity\index{Pythagorean identity} collapses to~$1$.}
...
\pair{02-introduction/simplification/4}{And a nested radical
...
\begin{codepairs}
\pair{02-introduction/calculus-series/1}{The exponential series, to sixth order.}
\pair{02-introduction/calculus-series/2}{$\arctan$: the alternating odd reciprocals whose
value at $x=1$ gives Leibniz's series for $\pi/4$.}
\pair{02-introduction/calculus-series/3}{At a pole the expansion is a \emph{Laurent}\index{Laurent series}
series---here $\cot x$ begins with a $1/x$ term.}
\pair{02-introduction/calculus-series/4}{With a \emph{symbolic} exponent the coefficients
come out as the generalized binomial coefficients.}
\pair{02-introduction/calculus-series/5}{\B{Normal} drops the $O[\,]$ term, turning the
...
\pair{02-introduction/calculus-integrate/4}{The Gaussian integral over the whole real\index{Gaussian integral}
