...
ever forming $7^{100}$.}
\pair{02-introduction/number-theory/7}{\B{Fibonacci} returns the fiftieth Fibonacci
...
you want. The \texttt{N}-prefixed family answers these questions numerically,
adaptively, and to machine precision:
...
Differential equations are solved numerically by \B{NDSolve}. We solve simple harmonic
motion, $y'' = -y$ with $y(0)=1$ and $y'(0)=0$, whose exact solution is $\cos x$:

\begin{codepairs}
\pair{02-introduction/numerical-calculus-ndsolve/1}{The solution comes back as an
\B{InterpolatingFunction}, evaluable anywhere in its range.}
\pair{02-introduction/numerical-calculus-ndsolve/2}{Evaluated at $\pi$ it gives
$-1$.}
\pair{02-introduction/numerical-calculus-ndsolve/3}{At $2\pi$ it gives $1$: the
numerical solution reproduces $\cos x$ exactly.}
\end{codepairs}
...
exactly and numerically. \B{Solve} finds \emph{exact} solutions---as integers,
radicals, or general closed forms---for a single equation or a whole system:
...
When an exact solution is unavailable---the general quintic has no formula in
radicals, by Abel's theorem---or simply not wanted, the numerical solvers take over:

\begin{codepairs}
\pair{02-introduction/solve-numerical/1}{\B{NSolve} finds all five roots of
$x^{5}-x-1$: one real, two complex-conjugate pairs, and no solution in radicals.}
\pair{02-introduction/solve-numerical/2}{\B{FindRoot}, started near $x=1$, converges
to the root of $\cos x = x$, the ``Dottie number.''}
\end{codepairs}
