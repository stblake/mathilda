...
comes back as an \B{InterpolatingFunction}, and \B{Plot} takes that directly, so the
motion---a decaying oscillation settling to rest---is one further command away:

\plotcell{02-introduction/ndsolve-solution}

\noindent The plot above is genuinely the \texttt{Out[2]} of that \B{Plot} command. To
keep it, \B{Export} writes any such graphic to PDF, PNG, or JPEG
(\texttt{Export["decay.pdf", plot]})---indeed every figure in this book is produced
exactly this way, by the running system.
