...
Differential equations are solved numerically by \B{NDSolve}, even nonlinear ones with no
closed-form solution. We solve the \emph{van der Pol oscillator}
$y'' + \mu\,(y^{2}-1)\,y' + y = 0$ (with $\mu = 3$, $y(0)=1$, $y'(0)=0$), whose
self-sustaining \emph{relaxation oscillations} settle onto a limit cycle:

\begin{codepairs}
\pair{02-introduction/numerical-calculus-ndsolve/1}{The solution comes back as an
\B{InterpolatingFunction}, evaluable anywhere on its range $[0, 20]$.}
\pair{02-introduction/numerical-calculus-ndsolve/2}{Its value at $x=5$, on the slow rise
of a cycle.}
\pair{02-introduction/numerical-calculus-ndsolve/3}{And at $x=20$, near the bottom of the
next plunge---no elementary formula gives these.}
\end{codepairs}

Rather than read the motion off a column of numbers, plot it. \B{Plot} accepts the
interpolating solution directly, and \B{Export} writes the result to a PDF---indeed every
figure in this book is produced exactly this way:

\bookfigure{02-introduction/ndsolve-solution}{The \B{NDSolve} solution of the van der Pol
oscillator over $[0, 20]$, plotted with \B{Plot} and written to PDF with
\texttt{Export["vdp.pdf", plot]}. The steep plunges and slow recoveries are the
oscillator's characteristic relaxation cycles---a shape no closed form captures.}
