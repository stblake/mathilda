...
\pair{03-introduction/elementary-numeric/3}{Ask for forty digits and you get \texttt{Sin[1]}
to that precision.}
...
\pair{03-introduction/algebra-groebner/2}{Naming a variable to eliminate in the third
argument switches \B{GroebnerBasis} to an \emph{elimination
order}\index{monomial order!elimination} (equivalently
\texttt{MonomialOrder\,->\,EliminationOrder}) and collapses the system to the curve's
implicit equation $x^{3}=y^{2}$, the cuspidal cubic. Section~\ref{sec:algebra} returns to
Gr\"obner bases.}
\end{codepairs}
...
\label{ssec:intro-systems}

A single equation has one unknown to pin down; a \emph{system}\index{system of equations}
couples several. The idea behind every method for solving one is \emph{elimination}: reduce
the system, a variable at a time, until a single equation in a single unknown remains, then
back-substitute. Mathilda offers a tool suited to each shape a system takes.

For a \emph{linear} system\index{system of equations!linear} the matrix tools do it
directly. \B{LinearSolve} returns the solution vector; \B{RowReduce} performs the Gaussian
elimination\index{Gaussian elimination} behind it, reducing the augmented matrix until the
answer can be read straight off the last column:

\begin{codepairs}
\pair{03-introduction/solve-systems/1}{The system $x+y=3,\ x-y=1$, solved as a matrix
equation: \B{LinearSolve} gives $(x,y)=(2,1)$.}
\pair{03-introduction/solve-systems/2}{The same system's augmented matrix, reduced to
row-echelon form\index{row echelon form}: the identity on the left leaves $x=2,\ y=1$ in
the last column. Section~\ref{sec:linalg} takes this further.}
\end{codepairs}

For a \emph{polynomial} system\index{system of equations!polynomial} \B{Solve} is the front
door, but the elimination machinery beneath it is worth seeing. \B{GroebnerBasis} rewrites
the system into a triangular one---a quadratic in $y$ alone, then $x$ in terms of
$y$---while \B{Resultant} eliminates a variable from a \emph{pair} of polynomials outright:

\begin{codepairs}
\pair{03-introduction/solve-systems/3}{The unit circle meets the line $y=x$ in two points,
returned exactly by \B{Solve}.}
\pair{03-introduction/solve-systems/4}{The same system's \B{GroebnerBasis} is triangular:
$2y^{2}-1$ involves $y$ alone, and $x-y$ then recovers $x$.}
\pair{03-introduction/solve-systems/5}{\B{Resultant} eliminates $y$ from the two
polynomials, leaving $2x^{2}-1=0$ in $x$ alone---the other half of the same triangulation.}
\end{codepairs}

When the answer is not a short list of points but a whole region, or you want every
degenerate branch kept, \B{Reduce} returns the complete solution set as a logical statement
rather than a list of rules---and, like \B{Solve}, it reads a list of equations as their
conjunction:

\begin{codepairs}
\pair{03-introduction/solve-systems/6}{The same intersection as a logical
description---a disjunction naming both points.}
\end{codepairs}

Systems come in kinds beyond the algebraic. A system of simultaneous
\emph{congruences}\index{congruence!system of}---$x\equiv2\ (3)$, $x\equiv3\ (5)$,
$x\equiv2\ (7)$---is solved by \B{ChineseRemainder}, which reconstructs the unique residue
modulo the moduli's product (\S\ref{sec:numbertheory}):

\begin{codepairs}
\pair{03-introduction/solve-systems/7}{The unique residue $23$ in $[0,105)$.}
\end{codepairs}

The exact algebra of systems---Gr\"obner bases, elimination, and \B{Reduce} over the
reals---is taken up in \S\ref{ssec:alg-solve-systems} and~\S\ref{ssec:alg-reduce}, where
\B{Eliminate} and the numerical \B{NSolve} join the toolkit.
...
Where \texttt{Simplify} \emph{searches} for the smallest form, sometimes you would rather
apply a fact directly and see its consequence. \B{Refine} does exactly that---the same
assumption-driven rewrites, without the search:

\begin{codepairs}
\pair{03-introduction/simplification-refine/1}{Under reality alone, $\sqrt{x^2}$ refines
to $|x|$.}
\pair{03-introduction/simplification-refine/2}{And $\sin(n\pi)$ collapses to $0$ once $n$
is known to be an integer.}
\end{codepairs}

\noindent Refine also \emph{decides} equations, inequalities, and domain memberships that
its assumptions settle; Section~\ref{ssec:alg-refine} takes it up in depth.
...
\texttt{SeriesData} object\index{SeriesData} into an ordinary polynomial.}
\end{codepairs}
...
Give Mathilda a list of numbers and it will describe them. The descriptive
statistics\index{statistics!descriptive} are ordinary functions of a list, and---like everything else---they
stay exact while the data is exact. Here are ten exam scores:

\begin{codepairs}
\pair{03-introduction/statistics/1}{Ten scores out of a hundred.}
\pair{03-introduction/statistics/2}{\B{Mean} is exact on integer data---the
rational $153/2$, not a rounded $76.5$.}
\pair{03-introduction/statistics/3}{\B{Median}, the middle value once sorted.}
\pair{03-introduction/statistics/4}{\B{StandardDeviation} measures the spread; \B{N}
turns the exact surd into a decimal.}
\pair{03-introduction/statistics/5}{\B{Quartiles} returns the lower quartile,
median, and upper quartile at once.}
\end{codepairs}

With a second measurement on each student---the hours they studied---\B{Correlation}
asks whether the two rise together, on a scale from $-1$ to $1$:

\begin{codepairs}
\pair{03-introduction/statistics/6}{Hours studied, same ten students.}
\pair{03-introduction/statistics/7}{A correlation of $0.98$: study time and score
move almost in lockstep.}
\pair{03-introduction/statistics/8}{And \B{MovingAverage} smooths a series by
averaging a sliding window---here of three.}
\end{codepairs}

Section~\ref{sec:statistics} treats location and spread, the five-number summary,
moments, correlation, moving statistics for time series, and the bridge from
data to probability distributions in full.
