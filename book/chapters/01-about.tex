...
Mathilda is a computer algebra system---a program that does mathematics
\emph{symbolically}, computing with $x$ and $\pi$ and $\sqrt{2}$ as exact objects
...
on the core architecture and evaluation semantics of the Wolfram Language, and it
is written in C and in its own language.

This chapter is about \emph{why} Mathilda exists, what it is built on, and why it
is written the way it is. The mathematics starts in earnest in the next chapter;
...
by the Risch algorithm, solving Diophantine equations, computing with Gr\"obner
...
little use. Mathilda is built to be quick: exact arithmetic on arbitrary-precision
integers, a bytecode compiler for numeric inner loops, and transparently packed
...
\emph{mathematical integrity}. When a system tells you that
\[
...
Openness also buys \emph{longevity}. Proprietary formats and products are
...
\begin{itemize}
  \item \textbf{GMP}~\cite{gmp}, the GNU Multiple Precision Arithmetic Library, provides the
  ...
  \item \textbf{MPFR}~\cite{mpfr} extends that idea to real numbers, giving correctly rounded
        arbitrary-precision floating point---so \B{N}\texttt{[expr, 100]} can be
        trusted to a hundred digits.
  \item \textbf{FLINT}~\cite{flint} accelerates polynomial arithmetic, factoring, and the
        rigorous numerics behind many special functions.
  \item \textbf{LAPACK} and \textbf{BLAS} (here, Apple's Accelerate framework)
  ...
  \item \textbf{GMP-ECM} supplies the Elliptic Curve Method for the harder cases
        of integer factorization.
  \item \textbf{Raylib} renders the interactive graphics produced by \B{Plot} and
        \B{Show}, and \textbf{GNU~Readline} gives the interactive session its
...
written in C99, and the choice is deliberate.
...
language models are exceptionally strong at writing and reasoning about C: it is
...
\begin{itemize}
  \item \textbf{Symbolic integration.} A complete implementation of the
        ...
        for integration in closed form, rivalling the Scratchpad / AXIOM / FriCAS
  ...
  \item \textbf{Algebraic geometry and commutative algebra.} Gr\"obner bases,
        ...
        that rivals Magma, Singular, and Macaulay2.
  \item \textbf{Number theory.} Factorization, primality proving, elliptic curves,
        and class-group computation to rival PARI/GP.
  ...
  \item \textbf{Tensor computation.} Manipulation of tensors and the objects of
        differential geometry that is as expressive and as efficient as Cadabra.
  \item \textbf{Arbitrary-precision numerics.} Correctly rounded and rigorously
        bounded computation---ball arithmetic in the spirit of Arb and mpmath---so a
  ...
  \item \textbf{Numerical computing.} Dense linear algebra, quadrature, and
        differential-equation solving at the level of LAPACK, SciPy, and Julia.
  \item \textbf{Optimization.} Local and global optimization---constrained,
        unconstrained, and mixed-integer---to rival SciPy and NLopt.
  \item \textbf{Array numerics.} Element-wise and reduction operations on dense,
        packed numerical arrays that are at least as fast as NumPy on a single
  ...
  \item \textbf{Compilation.} A bytecode compiler whose numeric throughput rivals
        Pipi, so that an inner loop written in Mathilda runs at the speed of compiled
