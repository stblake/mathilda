...
\emph{guaranteed enclosures}\index{interval arithmetic!enclosure} rather than points.
...
Rather than test the head by hand, you use the \emph{predicates}---the functions ending\index{predicate (Q function)}
...
\pair{arithmetic/predicates/4}{A Gaussian integer is an \emph{exact} number\index{Gaussian integer}, so
...
53-bit mantissa)\index{mantissa}.}
...
lowest wins---machine precision being the floor\index{machine precision!as floor}. This, too, follows Mathematica:
...
\emph{point} interval---\texttt{Interval[1.]}\index{interval arithmetic!point interval} is \texttt{Interval[\{1., 1.\}]}---rather
...
computer-assisted proof that $\sin$ has a root\index{computer-assisted proof} in $[245/78, 355/113]$, pinning $\pi$ to a
far tighter interval than $[3, 7/2]$ would.}
\end{codepairs}
