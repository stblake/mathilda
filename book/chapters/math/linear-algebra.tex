...
\calloutnote{Theory}{There is no perfect \texttt{ZeroTest}, and this is a theorem,
not a limitation of effort. \emph{Richardson's theorem}\index{Richardson's theorem}
shows that for expressions built from the rationals, \(\pi\), \(\log\), \(\exp\),
sines, and absolute values, deciding whether an expression is identically zero is
\emph{undecidable}. Every computer algebra system therefore chooses a zero test
that is sound but incomplete---it never calls a non-zero zero, but may fail to
prove a genuine zero. Making that choice a user-supplied option is the honest
design: you decide how hard to work at each pivot, trading speed against the
strength of the test, with \B{Simplify} or \B{PossibleZeroQ} at the strong end.}
