...
- `Mathilda_spec.md` — document `Factorial2` and the new Simplify capability.
...
full Mathilda test suite remains green: 94/94 binary tests pass with
no regressions.
...
- Spec docs (`Mathilda_spec.md`): ~110 LOC.
- Lessons (`tasks/lessons.md`): ~60 LOC.

**Surprises and lessons (saved to `tasks/lessons.md`):**

1. Mathilda's `Factor` *changes behaviour* inside Simplify
...
2. Mathilda's evaluator does NOT auto-coalesce
...
3. Mathilda's `Together` always expands the polynomial denominator
...
- [ ] Update `Mathilda_spec.md` with a new section "Square-root denesting
...
- The Mathilda evaluator re-merges `Times[Power[m, q], Power[n, -q]]` for
...
- Case 5's identity `(2+sqrt(5))^(1/3) + (2-sqrt(5))^(1/3) = 1` only
  holds under REAL cube-root semantics. Mathilda's principal-branch
...
worked). The full Mathilda test suite remains green; no regressions.
...
- The Mathilda evaluator re-merges `Times[Power[m, q], Power[n, -q]]`
...
- Case 5 holds only under real-cube-root semantics; Mathilda's primitive
